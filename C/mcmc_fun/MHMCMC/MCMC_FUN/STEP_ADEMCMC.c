...
double STEP_ADEMCMC(double *PARS, double *pars_new, PARAMETER_INFO PI, int C,
                    int NC, double *gratio) {
  ...
  */
  ...
  if (withinlim == 1) {
    ...
    */
    ...
    gc2n =
        -0.5 * (sumgamma * sumgamma * Dimo_a + sumgamma2 * (Dimd_a - Dimo_a));

    gn2c =
        -0.5 * log(prodgamma * prodgamma) -
        0.5 * (isumgamma * isumgamma * Dimo_a + isumgamma2 * (Dimd_a - Dimo_a));
