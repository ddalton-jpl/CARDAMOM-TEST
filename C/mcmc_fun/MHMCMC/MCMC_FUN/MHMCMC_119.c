...
double *MHMCMC_119(double(MODEL_LIKELIHOOD)(DATA, double *), DATA DATA,
  ...
  PARSALL = calloc(ceil((double)MCO.nOUT / (double)MCO.nADAPT) * PI.npars,
                   sizeof(double));
  ...
  if (isinf(P0) == -1) {
    printf("WARNING! P0=-inf - MHMCMC may get stuck - if so, please check your "
           "initial conditions\n");
  }
  ...
  while (N.ITER < MCO.nOUT) {
    ...
    if (MCO.nPRINT > 0 && N.ITER % MCO.nPRINT == 0) {
      ...
      printf("Total within range = %3.3f%%\n",
             (double)wrloc / (double)MCO.nPRINT * 100);
      wrloc = 0;
      printf("Total Accepted = %d\n", N.ACC);
      printf("Local Acceptance rate (acc,accloc,mco.nadapt) %3.1f%% (%8i, %8i, "
             "%8i)\n",
             N.ACCRATE * 100, N.ACC, N.ACCLOC, MCO.nADAPT);
      printf("Log Likelihood %e\n", P0);
      meanstepsize = 0;
      for (n = 0; n < PI.npars; n++) {
        meanstepsize += sqrt(N.parcovariance[n][n]) / (double)(PI.npars);
      }
