...
double *ADEMCMC(double(MODEL_LIKELIHOOD)(DATA, double *), DATA DATA,
                PARAMETER_INFO PI, MCMC_OPTIONS MCO, MCMC_OUTPUT *MCOUT) {
  ...
  if (MCO.APPEND == 0 && MCO.nWRITE > 0) {
    FILE *fileout = fopen(MCO.outfile, "wb");
    fclose(fileout);
  }
  ...
  /*PARSALL=calloc(MCO.nADAPT*PI.npars*NC,sizeof(double));*/ /*[DEMCMC: can
                                                                comment this
                                                                out]*/
  ...
  for (nn = 0; nn < NC; nn++) {
    for (n = 0; n < PI.npars; n++) {
      ...
      /*ONLY assigning randompars if (a) randparini==1 or (b)
       * PI.parini[n]=-9999*/

      if (MCO.randparini == 1 && PI.parfix[n] != 1) {
        /*random parameter if PI.parini = -9999*/
        PARS[n + nn * PI.npars] =
            nor2par((double)random() / RAND_MAX, PI.parmin[n], PI.parmax[n]);
      } else {
        par = PI.parini[n + nn * PI.npars];
        PARS[n + nn * PI.npars] = par;
        if (par > PI.parmax[n] | par < PI.parmin[n]) {
          printf("Warning, prescribed initial parameters are out of range");
        }
      }
    }
  }
  ...
  */
  ...
  for (nn = 0; nn < NC; nn++) {
    /*NOTE: passing pointer of PARS0 + N-chains: also use *(P+N) format if this
     * one won't work*/
    P[nn] = MODEL_LIKELIHOOD(DATA, &PARS[nn * PI.npars]);
    /*treating NaN as -inf*/
    if (isnan(P[nn])) {
      printf("Warning: MLF generated NaN... treating as -Inf");
      P[nn] = log(0);
    }
    if (Pmin > P[nn]) {
      Pmin = P[nn];
    ...
    if (isinf(P[nn]) == -1) {
      printf("WARNING! P(0)=-inf - MHMCMC may get stuck - if so, please check "
  ...
  for (N.ITER = 0; N.ITER < MCO.nOUT; N.ITER++) {
    ...
    /*UPDATE: retaining parameter vector (as done in ter Braak, 2006) and moving
     * on to next chain if metropolis ratio is rejected*/
    ...
    for (nn = 0; nn < NC; nn++) {
      ...
      // Standard DEMCMC
        ...
        PI.stepsize[0] = 1 - (1 - 2.38 / sqrt(2 * PI.npars) / 10) *
                                 (double)((double)(random() / RAND_MAX) < 0.9);
      ...
      // if (P_new-P[nn]+gratio>lr || (isinf(P_new)==0 && isinf(P[nn]) &&
      // withinrange==1) ){N.ACC=N.ACC+1;
      if (P_new - P[nn] + gratio > lr || (isinf(P[nn]) && withinrange == 1)) {
        N.ACC = N.ACC + 1;
        if (isinf(P_new) == 0 && isinf(P[nn])) {
          printf("pnew = %2.1f, p = %2.1f, (P_new-P[nn]) = %2.1f\n", P_new,
                 P[nn], P_new - P[nn]);
        }
        ...
        if (P_new >= BESTP[nn]) {
          for (n = 0; n < PI.npars; n++) {
            BESTPARS[n + nn * PI.npars] = pars_new[n];
            BESTP[nn] = P_new;
          }
          if (P_new == 0 && P_new > P[nn]) {
            printf("Found bestpars, prob = %2.1f, chain = %i\n", BESTP[nn], nn);
          }
        }
        P[nn] = P_new;
      }
  ...
  // printf("bestpars\n");
  for (n = 0; n < PI.npars * NC; n++) {
    MCOUT->best_pars[n] = BESTPARS[n];
