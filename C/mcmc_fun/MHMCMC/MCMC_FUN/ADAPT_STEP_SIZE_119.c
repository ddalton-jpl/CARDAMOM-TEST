...
int statfun_incremental_cov_mat(double **CM, int Npars, int Nsamples,
                                double *CMmean, double *newsample, double ar) {
  ...
  for (p = 0; p < Npars; p++) {
    for (pp = 0; pp < Npars; pp++) {
      CM[p][pp] = CM[p][pp] * (N - 1) / (N - 1 + ar) +
                  (N * CMmean[p] * CMmean[pp] -
                   (N + ar) * CMmeannew[p] * CMmeannew[pp] +
                   ar * newsample[p] * newsample[pp]) /
                      (N - 1 + ar);
    }
  }
...
int ADAPT_STEP_SIZE_119(double *PARSALL, PARAMETER_INFO PI, COUNTERS *N,
                        MCMC_OPTIONS MCO) {
  ...
  if (N->ACCLOC > PI.npars * 10 & N->covsamplecount == 0) {
    ...
    for (p = 0; p < PI.npars; p++) {
      for (n = 0; n < (N->ACCLOC - os); n++) {
        ...
        norparvec[p][n] = par2nor(PARSALL[PI.npars * (n + os) + p],
                                  PI.parmin[p], PI.parmax[p]);
      }
    }
    ...
    for (p = 0; p < PI.npars; p++) {
      for (pp = 0; pp < PI.npars; pp++) {
        N->parcovariance[p][pp] =
            covariance(norparvec[p], norparvec[pp], N->ACCLOC - os);
      }
    }
  ...
  /*statfun_incremental_cov_mat(double **CM, int Npars, int Nsamples, double
   *CMmean, double *newsample, double ar)*/
    ...
    for (p = 0; p < PI.npars; p++) {
      norparveci[p] = par2nor(PARSALL[PI.npars * (isample - 1) + p],
                              PI.parmin[p], PI.parmax[p]);
    }
    ...
    statfun_incremental_cov_mat(N->parcovariance, PI.npars, N->covsamplecount,
                                N->parmean, norparveci, 1);
