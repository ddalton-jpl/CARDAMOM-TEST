...
int STEP_DEMCMC(double *PARS, double *pars_new, PARAMETER_INFO PI, int C,
                int NC) {
  ...
  for (n = 0; n < PI.npars; n++) {
    npar[n] = par2nor(PARS[C * PI.npars + n], PI.parmin[n], PI.parmax[n]);
  }
  ...
  /*first: pick c1 and c2, make sure that (a) they are between 1 and NC, and (b)
   * they are not C*/
  c1 = C;
  c2 = C;
