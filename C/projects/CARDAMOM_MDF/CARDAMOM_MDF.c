...
int READ_MCOPT(MCMC_OPTIONS *MCOPT, DATA DATA, char *outfile) {
  ...
  if (DATA.ncdf_data.MCMCID.nSAMPLES == DEFAULT_INT_VAL) {
    DATA.ncdf_data.MCMCID.nSAMPLES = 2000;
  }
  if (MCOPT->nOUT == DEFAULT_INT_VAL) {
    MCOPT->nOUT = 10000;
  }
  if (MCOPT->nPRINT == DEFAULT_INT_VAL) {
    MCOPT->nPRINT = 1000;
  }
  if (MCOPT->minstepsize == DEFAULT_DOUBLE_VAL) {
    MCOPT->minstepsize = 1e-5;
  }
  if (MCOPT->mcmcid == DEFAULT_INT_VAL) {
    MCOPT->mcmcid = 119;
  }
  if (MCOPT->nADAPT == DEFAULT_INT_VAL) {
    MCOPT->nADAPT = 100;
  }
  if (MCOPT->fADAPT == DEFAULT_DOUBLE_VAL) {
    MCOPT->fADAPT = 0.05;
  }
  ...
  strcpy(stepfile, outfile);
  strcpy(startfile, outfile);
...
int main(int argc, char *CLA[]) {
  ...
  DATA DATA;
  /*Initialize data structure - this function is found in
  ...
  if (DATA.ncdf_data.MCMCID.seed_number == DEFAULT_INT_VAL) {
    DATA.ncdf_data.MCMCID.seed_number = 0;
  }
  ...
  /*NOTE  : READ_PARI_DATA function is stored in
   * DALEC_CDEA_TEMPLATE/MCMC_SETUP/MCMC_MODULES.c*/
  /*TO DO : (a) read DATA first - note that this includes model specific fields,
     such as nomet, nopars, etc. these are all loaded via the
     CARDAMOM_MODEL_LIBRARY(DATA) function*/
  ...
  /*the MLF function determines the probability of any model output based on
   * data and/or other constraints*/
  ...
  /*printf("CARDAMOM_MDF.c: DEMCMC temporarily disconnected, need to de-bug,
  correct and re-introduce"); printf("CARDAMOM_MDF.c: completed DEMCMC\n");
  break;*/
  default:
