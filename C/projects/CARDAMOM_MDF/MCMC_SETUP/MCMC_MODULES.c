...
int READ_PARI_DATA(PARAMETER_INFO *PI, DATA *DATA, MCMC_OUTPUT *MCOUT,
                   MCMC_OPTIONS *MCOPT, char *CLA[]) {
  /*READING IN DALEC_SYNTHETIC DATA*/
  /*opening file*/

  /*CHANGE4EDDIE*/
  /*this only applies to the native environment - change accordingly to add
   * default file!*/
  char filename[1000];
  strcpy(filename, CLA[1]);
  ...
  for (n = 0; n < PI->npars; n++) {
    PI->stepsize[n] = 0.0001;
  }

  printf(
      "CARDAMOM_MDF/MCMC_SETUP/MCMC_MODULES.c: Done with initial parameters");
...
int MEMORY_CLEANUP(DATA DATA, PARAMETER_INFO PI, MCMC_OPTIONS MCOPT,
                   MCMC_OUTPUT MCOUT) {
...
int INITIALIZE_PI_STRUCT(PARAMETER_INFO *PI, DATA *DATA, MCMC_OPTIONS *MCO) {
  oksofar("CARDAMOM_MDF/MCMC_SETUP/MCMC_MODULES.c INITIALIZE_PI_STRUCT():  "
          "initializing PI stucture");
