...
int FIND_EDC_INITIAL_VALUES(DATA CARDADATA, PARAMETER_INFO *PI,
                            MCMC_OPTIONS *MCOPT_CARDAMOM) {
  ...
  MCOPT.nWRITE = 0;
  /*randparini = 0*/
  /*this means all PI.parini values must either be given values or entered as
   * -9999*/
  MCOPT.randparini = 1;
  MCOPT.returnpars = 1;
  ...
  if (MCOPT_CARDAMOM->mcmcid == 3) {
    ...
    MCOPT.nOUT =
        CARDADATA.ncdf_data.MCMCID.nSAMPLES_EDC_SEARCH; /*Default =  20000*/
  ...
  printf("C/projects/CARDAMOM_MDF/MCMC_SETUP/PROJECT_FUN/"
         "FIND_EDC_INITIAL_VALUES.c: MCOUT structure initialized\n");
...
/*if number of parameters x number of chains available are contained in file,
 * then OK*/
...
int filelength;
if (fileout0 != NULL) {
  fseek(fileout0, 0, SEEK_END);
  filelength = ftell(fileout0) / sizeof(double);
  fclose(fileout0);
} else {
  filelength = 0;
}
...
/*Sampling M=N-N0 chains, where N0 is the number of existing chains in the
 * file*/
...
if (PI->npars > filelength) {
  ...
  for (m = 0; m < 1; m++) {
    ...
    while (PEDC != 0) {
      ...
      if (MCOPT.mcmcid == 119) {
        MHMCMC_119(CARDADATA.EMLF, CARDADATA, *PI, MCOPT, &MCOUT);
      };
      if (MCOPT.mcmcid == 2) {
        DEMCMC(CARDADATA.EMLF, CARDADATA, *PI, MCOPT, &MCOUT);
      };
      if (MCOPT.mcmcid == 3) {
        ADEMCMC(CARDADATA.EMLF, CARDADATA, *PI, MCOPT, &MCOUT);
      };
      ...
      for (nn = 0; nn < MCOPT.nchains; nn++) {
        PEDC = CARDADATA.EMLF(CARDADATA, PI->parini + nn * PI->npars);
        double P;
      ...
      for (nnn = 0; nnn < CARDADATA.noedcs; nnn++) {
        double prcnt = 100 * (double)CARDADATA.EDC_PASS_COUNTER[nnn] /
                       (double)CARDADATA.EDC_INSTANCE_COUNTER[nnn];
        printf("EDC no %i; attempts = %i; passes = %i (%2.2f%%);\n", nnn,
               CARDADATA.EDC_INSTANCE_COUNTER[nnn],
               CARDADATA.EDC_PASS_COUNTER[nnn], prcnt);
      }
      printf("*******\n");
      printf("*******\n");
      ...
      if (MCOPT.mcmcid == 2 && PEDCC > MCOPT.nchains) {
        PEDC = 0;
      }
      // Guarantee that at least half of chains have non-zero starting
      // probabilities
      if (MCOPT.mcmcid == 3) {
        if (PEDCC > nstartchains) {
          PEDC = 0;
        } else {
          PEDC = -1;
        }
      }
      if (MCOPT.mcmcid == 2 || MCOPT.mcmcid == 3) {
        MCOPT.randparini = 0;
      }
      /*Hard coding*/

      /*in case one EDC missing*/
      if (MCOPT.mcmcid == 119 && PEDC != 0 && count % 3 == 0) {
        for (n = 0; n < PI->npars; n++) {
    ...
    for (n = 0; n < PI->npars * MCOPT.nchains; n++) {
      fwrite(&MCOUT.best_pars[n], 1, sizeof(double), fileout);
    }
    fclose(fileout);
...
fread(PI->parini, sizeof(double), PI->npars * MCOPT_CARDAMOM->nchains,
      fileout0);
fclose(fileout0);
...
PI->parini)); printf("Probability of starting parameters =
