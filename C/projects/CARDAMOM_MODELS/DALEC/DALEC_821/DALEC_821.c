...
int DALEC_821(DATA DATA, double const *pars) {
  ...
  */
  ...
  for (n = 0; n < N_timesteps; n++) {
    ...
    double *BALDOCCHI_ANALYTICAL_OUTPUT =
        BALDOCCHI_ANALYTICAL(met_list, var_list);

    /*GPP*/
    /* GPP * soil water limitation */
    FLUXES[f + 0] =
        BALDOCCHI_ANALYTICAL_OUTPUT[0] * fmin(POOLS[p + 6] / pars[23], 1);
    /*Evapotranspiration (VPD = DATA.MET[m+7])*/
    /* ET * soil water limitation */
    FLUXES[f + 28] =
        BALDOCCHI_ANALYTICAL_OUTPUT[1] * fmin(POOLS[p + 6] / pars[23], 1);
    ...
    /* x (1 + a* P/P0)/(1+a)*/
    FLUXES[f + 1] =
        exp(pars[9] * (0.5 * (T2M_MIN[n] + T2M_MAX[n]) - meantemp)) *
        ((PREC[n] / meanprec - 1) * pars[30] + 1);
    /*respiration auto*/
    ...
    /*labile production*/
    FLUXES[f + 4] =
        (FLUXES[f + 0] - FLUXES[f + 2] - FLUXES[f + 3]) * pars[12 - 1];
    /*root production*/
    FLUXES[f + 5] =
    ...
    FLUXES[f + 8] =
        (2 / sqrt(pi)) * (ff / wf) *
        exp(-pow(sin((TIME_INDEX[n] - pars[13] + osf) / sf) * sf / wf, 2));
    /*Labrelease factor*/
    FLUXES[f + 15] =
        (2 / sqrt(pi)) * (fl / wl) *
        exp(-pow(sin((TIME_INDEX[n] - pars[10] + osl) / sf) * sf / wl, 2));
    ...
    if (POOLS[p + 6] > pars[22] / 2) {
      FLUXES[f + 29] = (POOLS[p + 6] - pars[22] / 4) / deltat;
    }

    POOLS[nxp + 6] = POOLS[p + 6] - FLUXES[f + 29] * deltat + PREC[n] * deltat -
                     FLUXES[f + 28] * deltat;
