...
int EDC2_821(double const *pars, DATA DATA, struct EDCDIAGNOSTIC *EDCD) {
  ...
  */
  ...
  for (n = 0; n < nopools; n++) {
    ...
    /*of each constraint*/
    EDCD->EDCPROB[7 - 1 + n] =
        -0.5 * pow(log(Rs) / log(EQFnew), 2); /*-0.5*pow((Rs-Rm)/etol,2);*/
  ...
  if (((EDC == 1 & DIAG == 0) || DIAG == 1 ||
       (EDC == 1 & DIAG == 2 & EDCD->SWITCH[7 - 1 + n] == 1)) &
      (pars[23] > MPOOLS[6])) {
    EDC = ipow(0, EDCD->SWITCH[14 - 1]);
    EDCD->PASSFAIL[14 - 1] = 0;
  }
  ...
  if (gppcheck == 1) {
    ...
    if (EDC == 1 || DIAG == 1) {
      ...
      while ((EDC == 1 || DIAG == 1)) {
        ...
        PEDC = 1;
        while ((nn < N_timesteps + 1) & (PEDC == 1)) {
          /*printf("GPP = %2.2f\n",DATA.M_GPP[nn]);*/ /*Don't do this too many
                                                         print outs*/
