...
int EDC2_410(double const *pars, DATA DATA, struct EDCDIAGNOSTIC *EDCD) {
  ...
  for (n = 0; n < nopools; n++) {
    MPOOLS[n] = mean_pool(POOLS, n, nodays + 1, nopools);
  };
  ...
  for (n = 0; n < nopools; n++) {
    for (m = 0; m < (nodays / dint + 1); m++) {
      MPOOLSjan[n] =
          MPOOLSjan[n] + POOLS[nopools * (m * dint) + n] / (nodays / dint + 1);
    }
  }
  ...
  if (((EDC == 1 & DIAG == 0) || DIAG == 1 ||
      ...
      (MPOOLS[1] > MPOOLS[2] * 5 | MPOOLS[1] * 5 < MPOOLS[2])) {
    EDC = pow(0, EDCD->SWITCH[6 - 1]);
    EDCD->PASSFAIL[6 - 1] = 0;
  }
  ...
  /*for
   * (y=0;y<noyears;y++){MAPOOL[y]=mean_annual_pool(POOLS,y,n,nopools,deltat);}*/

  /*step 2 - checking pool growth*/
  /*if (((EDC==1 & DIAG==0) || DIAG==1) & (POOLS[n +
   * nopools*nodays]/POOLS[n]>G*(double)noyears)){EDC=0;EDCD->PASSFAIL[7-1]=0;}}*/
  /*if (((EDC==1 & DIAG==0) || DIAG==1 || (EDC==1 & DIAG==2 &
  EDCD->SWITCH[7-1]==1)) &
  (MAPOOL[noyears-1]/MAPOOL[0]>(1+G*(double)noyears))){EDC=pow(0,EDCD->SWITCH[7-1]);;EDCD->PASSFAIL[7-1]=0;}
  ...
  */
  ...
  for (n=0;n<nopools-4;n++){if ((EDC==1 & DIAG==0) || DIAG==1 || (EDC==1 &
  DIAG==2 & EDCD->SWITCH[8-1]==1)) {
  C=expdecay2(POOLS,n,nodays+1,deltat,nopools);
  if (fabs(-log(2)/C)<(365.25*cyears) & C<0 &
  isfinite(C)){EDC=pow(0,EDCD->SWITCH[8-1]);;EDCD->PASSFAIL[8-1]=0;}}};
  */
  ...
  double const fsom =
      fwood + (froot + flab + ffol) * pars[0] / (pars[0] + pars[7]);
  ...
  /*double meangpp=0;for
   * (n=0;n<nodays;n++){meangpp+=FLUXES[n*nofluxes]/(double)nodays;}*/
  ...
  for (n = 0; n < 6; n++) {
    if ((EDC == 1 || DIAG == 1) & ((MPOOLS[n]) > parmax[n + 17])) {
      EDC = 0;
