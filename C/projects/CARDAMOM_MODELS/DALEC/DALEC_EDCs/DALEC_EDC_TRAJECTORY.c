...
double DALEC_EDC_TRAJECTORY(DATA *DATA, void *EDCstruct) {
  ...
  for (s = 0; s < E.no_pools_to_check; s++) {
    ...
    int dint = (int)floor(
        N_timesteps / (TIME_INDEX[N_timesteps - 1] - TIME_INDEX[0]) * 365.25);
    ...
    for (m = 0; m < (N_timesteps / dint + 1); m++) {
      MPOOLSjan = MPOOLSjan +
                  POOLS[nopools * (m * dint) + p] / (N_timesteps / dint + 1);
    }
    ...
    for (i = 0; i < DALECmodel->SIOMATRIX[p].N_STATE_INPUT_FLUXES; i++) {
      Fin += FT[DALECmodel->SIOMATRIX[p].STATE_INPUT_FLUXES[i]];
    }
    for (i = 0; i < DALECmodel->SIOMATRIX[p].N_STATE_OUTPUT_FLUXES; i++) {
      Fout += FT[DALECmodel->SIOMATRIX[p].STATE_OUTPUT_FLUXES[i]];
    }
    ...
    //
    //         printf("p = %i\n",s);
    //         printf("-0.5*pow(log(Rs)/log(EQF),2) =
    //         %2.2f\n",-0.5*pow(log(Rs)/log(EQF),2)); printf("- 0.5
    //         *pow((Rs-Rm)/etol,2) = %2.2f\n",- 0.5 *pow((Rs-Rm)/etol,2));
    // //
    //
    //                         printf("Rs = %2.2f\n",Rs);
    //                         printf("Rm = %2.2f\n",Rm);
