...
double DALEC_EDC_NSC_ABGB_RATIO(DATA *DATA, void *EDCstruct) {
  ...
  DALEC_EDC_NSC_ABGB_RATIO_STRUCT E =
      *(DALEC_EDC_NSC_ABGB_RATIO_STRUCT *)EDCstruct;
  ...
  double M_NSC =
      mean_pool(DATA->M_POOLS, E.nsc_pool_index, N_timesteps + 1, nopools);

  double M_ALL_ELSE = 0;

  int n;
  for (n = 0; n < E.no_other_pool_indices; n++) {
    M_ALL_ELSE += mean_pool(DATA->M_POOLS, E.other_pool_indices[n],
                            N_timesteps + 1, nopools);
  }
