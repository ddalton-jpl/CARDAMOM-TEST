...
double DALEC_EDC_STATE_RANGES(DATA *DATA, void *EDCstruct) {
  ...
  double PEDC = 0;
  // Reading by casting void pointer to "DALEC_EDC_PARS_INEQUALITY_STRUCT"
  // format
  ...
  while (PEDC == 0 & p < DATA->nopools & k == 0) {
    ...
    while (PEDC == 0 & n < DATA->ncdf_data.TIME_INDEX.length & k == 0) {
      ...
      if (DATA->M_POOLS[p + n * DATA->nopools] < E.min_val[p] ||
          DATA->M_POOLS[p + n * DATA->nopools] > E.max_val[p]) {
        PEDC = -INFINITY;
        k = -1;
      }

      // if (p == 18){printf("E.max_val[p] = %2.2f, E.min_val[p] = %2.2f, SM =
  ...
  //         printf("pool %i; Attempt = %i, Pass = %i, Percent =
  //         %2.2f\%\n",p,attempt_count[p], pass_count[p],
  //         100*(double)pass_count[p]/(double)attempt_count[p]);
  //         }}
