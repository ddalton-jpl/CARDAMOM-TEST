...
double DALEC_EDC_START_TEMP(DATA *DATA, void *EDCstruct) {
  ...
  SOILTEMP.IN.soil_water = HYDROFUN_MOI2EWT(
      DATA->M_PARS[E.i_SM_idx], DATA->M_PARS[E.por_idx], DATA->M_PARS[E.z_idx]);
  SOILTEMP.IN.internal_energy = DATA->M_PARS[E.i_E_idx] * DATA->M_PARS[E.z_idx];
  ; // Joules
  ...
  if (SOILTEMP.OUT.TEMP<E.min_temp | SOILTEMP.OUT.TEMP> E.max_temp) {
    PEDC = -INFINITY;
  }
