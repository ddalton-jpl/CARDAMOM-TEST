...
double RUN_DALEC_EDCs(DATA *DATA, EDCs *EDCs, bool prerun) {
  ...
  // Continue from here: need to probably allow for EDCs to be standalone DATA
  // field to allow for more versatile code(maybe) later on
  ...
  for (n = 0; n < DATA->noedcs; n++) {
    ...
    if (EDCs[n].prerun == prerun) {
      ...
      if (P > -INFINITY) {
        ...
        // Add all probs up
        P += DATA->M_EDCs[n];
        // if (EDCs[n].prerun==0){      printf("n = %i; DATA->M_EDCs[n] = %2.2f;
        // P = %2.2f\n",n,DATA->M_EDCs[n],P);};

        // counting EDC passes
        if (isfinite(DATA->M_EDCs[n])) {
          DATA->EDC_PASS_COUNTER[n] += 1;
        }

        // printf("Inside run EDCs: DATA->M_EDCs[%i] =
        // %2.2f\n",n,DATA->M_EDCs[n] );
      }
      //              else {     if (EDCs[n].prerun==false){ printf("EDC fail =
      //              %i\n",n);}}
    }
  }
