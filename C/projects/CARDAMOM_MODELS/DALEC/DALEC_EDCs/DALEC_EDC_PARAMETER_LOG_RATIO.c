...
double DALEC_EDC_PARAMETER_LOG_RATIO(DATA *DATA, void *EDCstruct) {
  ...
  // format
  DALEC_EDC_PARAMETER_LOG_RATIO_STRUCT E =
      *(DALEC_EDC_PARAMETER_LOG_RATIO_STRUCT *)EDCstruct;

  double mod_ratio = log(DATA->M_PARS[E.numerator_index]) /
                     log(DATA->M_PARS[E.denominator_index]);
