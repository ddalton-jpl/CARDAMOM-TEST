...
int PREDERIVE_DALEC_1100_DATA(DALEC *DALECmodel, DATA *DATA) {
  ...
  for (n = 0; n < N_timesteps; n++) {
    ...
    double alpha =
        asin((sin(pi / 180 * LAT) * sin(pi / 180 * DA) +
              cos(pi / 180 * LAT) * cos(pi / 180. * DA) * cos(pi / 180 * h))) *
        180 / pi; // solar altitude
    double zenith_angle = 90 - alpha;

    // printf("SZA local = %2.2f, SZA global = %2.2f, SZA diff = %2.2f\n",
    // zenith_angle,DATA.ncdf_data.SZA.values,DATA.ncdf_data.SZA.values -
    // zenith_angle); double LAD = 1.0; //leaf angle distribution double VegK =
    // sqrt(pow(LAD,2)+
    // pow(tan(zenith_angle/180*pi),2))/(LAD+1.774*pow((1+1.182),-0.733));
    // //Campbell and Norman 1998

    double LAD =
        0.5; // leaf angle distribution// optimize leaf angle distribution.
...
int DALEC_1100_FLUX_SOURCES_SINKS(DALEC *DALECmodel) {
  ...
  int n;
  for (n = 0; n < DALECmodel->nofluxes; n++) {
    FIOMATRIX.SOURCE[n] = -1;
    FIOMATRIX.SINK[n] = -1;
  }
...
int DALEC_1100(DATA DATA, double const *pars) {
  ...
  double deltat =
      DATA.ncdf_data.TIME_INDEX.values[1] - DATA.ncdf_data.TIME_INDEX.values[0];
  double one_over_deltat = 1 / deltat;

  int N_timesteps = DATA.ncdf_data.TIME_INDEX.length;

  /*Pointer transfer - all data stored in fluxes and pools will be passed to
   * DATA*/
  ...
  /*water pools*/
  POOLS[S.H2O_LY1] =
      HYDROFUN_MOI2EWT(pars[P.i_LY1_SM], pars[P.LY1_por], pars[P.LY1_z]);
  POOLS[S.H2O_LY2] =
      HYDROFUN_MOI2EWT(pars[P.i_LY2_SM], pars[P.LY2_por], pars[P.LY2_z]);
  POOLS[S.H2O_LY3] =
      HYDROFUN_MOI2EWT(pars[P.i_LY3_SM], pars[P.LY3_por], pars[P.LY3_z]);
  POOLS[S.H2O_SWE] = pars[P.i_SWE];
  /*Energy pools*/
  // Step 1. derive temperature based on i_LY1_E (which will represent energy
  // per mm H2O). INTERNAL_ENERGY_PER_LIQUID_H2O_UNIT_MASS(infiltemp); Declare
  // stryct
  ...
  // double INITIALIZE_INTERNAL_SOIL_ENERGY(double internal_energy_per_mm_H2O,
  // double H2O_mm, double dry_soil_vol_heat_capacity, double depth )
  POOLS[S.E_LY1] = INITIALIZE_INTERNAL_SOIL_ENERGY(
      pars[P.i_LY1_E], POOLS[S.H2O_LY1], pars[P.LY1_vhc], pars[P.LY1_z]);
  POOLS[S.E_LY2] = INITIALIZE_INTERNAL_SOIL_ENERGY(
      pars[P.i_LY2_E], POOLS[S.H2O_LY2], pars[P.LY2_vhc], pars[P.LY2_z]);
  POOLS[S.E_LY3] = INITIALIZE_INTERNAL_SOIL_ENERGY(
      pars[P.i_LY3_E], POOLS[S.H2O_LY3], pars[P.LY3_vhc], pars[P.LY3_z]);
  ...
  POOLS[S.D_SM_LY1] = HYDROFUN_EWT2MOI(POOLS[S.H2O_LY1], pars[P.LY1_por],
                                       pars[P.LY1_z]); // soil moisture LY1
  POOLS[S.D_SM_LY2] = HYDROFUN_EWT2MOI(POOLS[S.H2O_LY2], pars[P.LY2_por],
                                       pars[P.LY2_z]); // soil moisture LY3
  POOLS[S.D_SM_LY3] = HYDROFUN_EWT2MOI(POOLS[S.H2O_LY3], pars[P.LY3_por],
                                       pars[P.LY3_z]); // soil moisture LY3
  // Convert to potential
  // Min psi ensures large negative psis not resolved by model needlessly
  double minpsi = -30;
  POOLS[S.D_PSI_LY1] =
      fmax(HYDROFUN_MOI2PSI(POOLS[S.D_SM_LY1], psi_porosity, pars[P.retention]),
           minpsi);
  POOLS[S.D_PSI_LY2] =
      fmax(HYDROFUN_MOI2PSI(POOLS[S.D_SM_LY2], psi_porosity, pars[P.retention]),
           minpsi);
  POOLS[S.D_PSI_LY3] =
      fmax(HYDROFUN_MOI2PSI(POOLS[S.D_SM_LY3], psi_porosity, pars[P.retention]),
           minpsi);
  ...
  // Populate with run-specific constrants
  // LY1
  LY1SOILTEMP.IN.dry_soil_vol_heat_capacity = pars[P.LY1_vhc];
  ;                                                // J/m3/K
  LY1SOILTEMP.IN.depth = pars[P.LY1_z];            // m
  LY1SOILTEMP.IN.soil_water = POOLS[S.H2O_LY1];    // mm (or kg/m2)
  LY1SOILTEMP.IN.internal_energy = POOLS[S.E_LY1]; // Joules
  ...
  // LY2
  LY2SOILTEMP.IN.dry_soil_vol_heat_capacity = pars[P.LY2_vhc];
  ;                                                // J/m3/K
  ...
  // LY3
  LY3SOILTEMP.IN.dry_soil_vol_heat_capacity = pars[P.LY3_vhc];
  ;                                                // J/m3/K
  ...
  //     double   NSC;//Clab
  //     double   LY1_SM;//m3/m3
  //     double   parameter1;//replace with any name, no constraints on naming
  //     convention double   parameter2;//replace with any name, no constraints
  //     on naming convention } IN; struct { double *    AUTO_RESP_MAINTENANCE;
  ...
  for (n = 0; n < N_timesteps; n++) {
    ...
    // double h = (AST-12*60)/4; //hour angle
    // double alpha =
    // asin((sin(pi/180*DATA.ncdf_data.LAT)*sin(pi/180*DA)+cos(pi/180*DATA.ncdf_data.LAT)*cos(pi/180.*DA)*cos(pi/180*h)))*180/pi;
    // //solar altitude double zenith_angle = 90-alpha;
    //
    // //printf("SZA local = %2.2f, SZA global = %2.2f, SZA diff = %2.2f\n",
    // zenith_angle,DATA.ncdf_data.SZA.values,DATA.ncdf_data.SZA.values -
    // zenith_angle);
    // //double LAD = 1.0; //leaf angle distribution
    // //double VegK = sqrt(pow(LAD,2)+
    // pow(tan(zenith_angle/180*pi),2))/(LAD+1.774*pow((1+1.182),-0.733));
    // //Campbell and Norman 1998
    //
    // double LAD = 0.5; //leaf angle distribution// optimize leaf angle
    // distribution. double VegK0 = LAD/cos(zenith_angle/180*pi);
    //

    DALEC *DALECmodel = (DALEC *)DATA.MODEL;

    DALEC_1100_DATA_STRUCT *DALEC_1100_DATA =
        (DALEC_1100_DATA_STRUCT *)DALECmodel->MODEL_DATA;

    double VegK = DALEC_1100_DATA->VegK[n];
    // CONTINUE FROM HERE(
    // printf("VegK internal, external = %2.2f,  %2.2f\n",VegK,
    // DALEC_1100_DATA->VegK[n]);
    ...
    // double psi_LY10 =
    // HYDROFUN_MOI2PSI(max(POOLS[p+S.D_SM_LY1],0),psi_porosity,pars[P.retention]);
    // double psi_LY10 =
    // HYDROFUN_MOI2PSI(POOLS[p+S.D_SM_LY1],psi_porosity,pars[P.retention_ly1]);
    double beta1 =
        1 /
        (1 + exp(pars[P.beta_lgr] *
                 (-1 * POOLS[p + S.D_PSI_LY1] / pars[P.psi_50] - 1))) *
        POOLS[p + S.D_LF_LY1];
    double beta2 =
        1 /
        (1 + exp(pars[P.beta_lgr] *
                 (-1 * POOLS[p + S.D_PSI_LY2] / pars[P.psi_50] - 1))) *
        POOLS[p + S.D_LF_LY2];
    double beta =
    ...
    if (beta1 > 0 || beta2 > 0) {
      FLUXES[f + F.transp1] =
          transp * beta1 * pars[P.LY1_z] /
          (beta1 * pars[P.LY1_z] + beta2 * pars[P.LY2_z] * pars[P.root_frac]);
      // FLUXES[f+F.transp2] =
      // transp*beta2*pars[P.LY2_z]*pars[P.root_frac]/(beta1*pars[P.LY1_z]+beta2*pars[P.LY2_z]*pars[P.root_frac]);
    ...
    POOLS[nxp + S.H2O_SWE] =
        POOLS[p + S.H2O_SWE] +
        FLUXES[f + F.snowfall] * deltat; /*first step snowfall to SWE*/
    // transient_SCF
    double SCFtemp =
        POOLS[nxp + S.H2O_SWE] / (POOLS[nxp + S.H2O_SWE] + pars[P.scf_scalar]);
    // Snow melt, based on new SWE
    double SNOWMELT =
        fmin(fmax((DGCM_TK0C + SKT[n] - pars[P.min_melt]) * pars[P.melt_slope],
                  0),
             1) *
        POOLS[nxp + S.H2O_SWE] * one_over_deltat; /*melted snow per day*/
    ...
    // FLUXES[f+F.melt]=SNOWMELT;
    //     //
    //     POOLS[nxp+S.D_SCF]=POOLS[nxp+S.H2O_SWE]/(POOLS[nxp+S.H2O_SWE]+pars[P.scf_scalar]);
    //     //snow cover fraction
    //
    // FLUXES[f+F.sublimation]=fmax(
    // pars[P.sublimation_rate]*VPD[n]*SRAD[n]*POOLS[nxp+S.D_SCF]);
    //

    /*Insure SWE does not go negative due to machine error*/
    POOLS[nxp + S.H2O_SWE] =
        fmax(POOLS[nxp + S.H2O_SWE] -
                 (FLUXES[f + F.melt] + FLUXES[f + F.sublimation]) * deltat,
             0);

    FLUXES[f + F.ets] = FLUXES[f + F.evap] + FLUXES[f + F.transp1] +
                        FLUXES[f + F.transp2] + FLUXES[f + F.sublimation];
    ...
    // Snow free
    double SWout_snowfree =
        (SWin * 0.5 * (pars[P.leaf_refl_par] + pars[P.leaf_refl_nir])); // W m-2
    // Weighted average of surface albedo considering SW snow albedo as 0.9
    double snow_albedo = 0.9; // Consider age-dependent albedo.

    // SW out
    double SWout = (1. - POOLS[p + S.D_SCF]) * SWout_snowfree +
                   POOLS[p + S.D_SCF] * (SWin * snow_albedo); // W m-2
    ...
    double LE =
        lambda *
        (FLUXES[f + F.evap] + FLUXES[f + F.transp1] + FLUXES[f + F.transp2]) /
        DGCM_SEC_DAY;               // W m-2
    FLUXES[f + F.latent_heat] = LE; // W m-2
    // specific heat capacity of dry air is 1.00464 KJ kg -1 K -1
    //  Consider surface pressure as forcing for more accurate conversion from
    //  mol to m3 Consider explicitly calculating cp based on humidity (derived
    //  from VPD and pressure)
    double cp = 29.2;   // J mol-1 K-1 representative specific heat of moist air
                        // at const pressure from Bonan book
    ...
    // Sensible heat
    //  double H = cp*(tskin_k - air_temp_k)*pars[P.ga]*moles_per_m3; // ga in m
    //  s-1, FLUXES[f+F.sensible_heat] = H; // W m-2
    // Ground heat flux ONLY for energy in&out of vegetation-soil continuum
    // Rn is scaled by snow free area, because we exclude snow energy balance
    // from energy ODEs H is scaled by snow free area, because we exclude snow
    // energy balance from energy ODEs LE is fully included, as evaporation and
    // transpiration are assumed to come fully from snow-free areas (caveat:
    // snow evaporation is a thing, btut we assume it's zero for this model)

    // Ideally, Rn should be snow-free vs snow-covered.

    // double  G = Rn*(1. - POOLS[p+S.D_SCF]) - H*(1. - POOLS[p+S.D_SCF]) - LE;
    // // W m-2 FLUXES[f+F.ground_heat] = G; // W m-2 FLUXES[f+F.gh_in] =
    // G*DGCM_SEC_DAY; // J m-2 d-1

    // Gh_in approach 2 based on soil and LST
    FLUXES[f + F.ground_heat] =
        (pars[P.thermal_cond_surf] * (tskin_k - POOLS[p + S.D_TEMP_LY1]) /
         (pars[P.LY1_z] * 0.5)) *
        (1. - POOLS[p + S.D_SCF]);
    FLUXES[f + F.gh_in] = FLUXES[f + F.ground_heat] * DGCM_SEC_DAY;
    // Using G, Rn and LE to derive H
    //  H = Rn - G  - LE
    FLUXES[f + F.sensible_heat] =
        Rn - FLUXES[f + F.ground_heat] - FLUXES[f + F.latent_heat];

    // Infiltration (mm/day)
    double liquid_in = (PREC[n] - SNOWFALL[n] + FLUXES[f + F.melt]);
    FLUXES[f + F.infil] =
        pars[P.max_infil] * (1 - exp(-liquid_in / pars[P.max_infil]));
    ...
    // double sm_LY1 =
    // HYDROFUN_EWT2MOI(POOLS[p+S.H2O_LY1],pars[P.LY1_por],pars[P.LY1_z]);
    // double sm_LY3 =
    // HYDROFUN_EWT2MOI(POOLS[p+S.H2O_LY3],pars[P.LY3_por],pars[P.LY3_z]);
    ...
    double drain_LY1 =
        POOLS[p + S.D_LF_LY1] * DRAINAGE(POOLS[p + S.D_SM_LY1],
                                         pars[P.Q_excess], -pars[P.field_cap],
                                         psi_porosity, pars[P.retention]);
    double drain_LY2 =
        POOLS[p + S.D_LF_LY2] * DRAINAGE(POOLS[p + S.D_SM_LY2],
                                         pars[P.Q_excess], -pars[P.field_cap],
                                         psi_porosity, pars[P.retention]);
    double drain_LY3 =
        POOLS[p + S.D_LF_LY3] * DRAINAGE(POOLS[p + S.D_SM_LY3],
                                         pars[P.Q_excess], -pars[P.field_cap],
                                         psi_porosity, pars[P.retention]);

    // Drainage becomes runoff from pools
    FLUXES[f + F.q_ly1] =
        HYDROFUN_MOI2EWT(drain_LY1, pars[P.LY1_por], pars[P.LY1_z]) *
        one_over_deltat;
    FLUXES[f + F.q_ly2] =
        HYDROFUN_MOI2EWT(drain_LY2, pars[P.LY2_por], pars[P.LY2_z]) *
        one_over_deltat;
    FLUXES[f + F.q_ly3] =
        HYDROFUN_MOI2EWT(drain_LY3, pars[P.LY3_por], pars[P.LY3_z]) *
        one_over_deltat;

    // Convert to conductivity
    double k_LY1 = HYDROFUN_MOI2CON(POOLS[p + S.D_SM_LY1], pars[P.hydr_cond],
                                    pars[P.retention]);
    double k_LY2 = HYDROFUN_MOI2CON(POOLS[p + S.D_SM_LY2], pars[P.hydr_cond],
                                    pars[P.retention]);
    double k_LY3 = HYDROFUN_MOI2CON(POOLS[p + S.D_SM_LY3], pars[P.hydr_cond],
                                    pars[P.retention]);

    // Calculate inter-pool transfer in m/s (positive is LY1 to LY2)
    double pot_xfer =
        1000 * sqrt(k_LY1 * k_LY2) *
        (1e-9 * (POOLS[p + S.D_PSI_LY1] - POOLS[p + S.D_PSI_LY2]) /
             (9.8 * 0.5 * (pars[P.LY1_z] + pars[P.LY2_z])) +
         1);
    ...
    if (pot_xfer > 0) { // Water is going LY1->LY2 (down)
      ...
      SPACEavail =
          fmax(pars[P.LY2_z] * pars[P.LY2_por] * 1e3 - POOLS[p + S.H2O_LY2] +
                   (FLUXES[f + F.q_ly2] + FLUXES[f + F.transp2]) * deltat,
               0);
      // Available water in LY1 (after runoff, et, and infiltration)
      H2Oavail = fmax(POOLS[p + S.D_LF_LY1] * POOLS[p + S.H2O_LY1] +
                          (FLUXES[f + F.infil] - FLUXES[f + F.q_ly1] -
                           FLUXES[f + F.evap] - FLUXES[f + F.transp1]) *
                              deltat,
                      0);
      // Maximum transfer flux in mm (actual transfer may be less due to water
      ...
      FLUXES[f + F.ly1xly2] =
          fmin(Max_H2O_xfer, fmin(SPACEavail, H2Oavail)) * one_over_deltat;
      ...
      SPACEavail =
          fmax(pars[P.LY1_z] * pars[P.LY1_por] * 1e3 - POOLS[p + S.H2O_LY1] -
                   (FLUXES[f + F.infil] - FLUXES[f + F.q_ly1] -
                    FLUXES[f + F.evap] - FLUXES[f + F.transp1]) *
                       deltat,
               0);
      ...
      H2Oavail =
          fmax(POOLS[p + S.D_LF_LY2] * POOLS[p + S.H2O_LY2] -
                   (FLUXES[f + F.q_ly2] + FLUXES[f + F.transp2]) * deltat,
               0);
      // Maximum transfer flux in mm (actual transfer may be less due to water
      // or space availability)
    ...
    pot_xfer = 1000 * sqrt(k_LY2 * k_LY3) *
               (1e-9 * (POOLS[p + S.D_PSI_LY2] - POOLS[p + S.D_PSI_LY3]) /
                    (9.8 * 0.5 * (pars[P.LY2_z] + pars[P.LY3_z])) +
                1);
    double TEMPxfer_2to3;
    if (pot_xfer > 0) { // Water is going LY2->LY3 (down)
      // Available space in LY3 (after runoff)
      SPACEavail = fmax(pars[P.LY3_z] * pars[P.LY3_por] * 1e3 -
                            POOLS[p + S.H2O_LY3] + FLUXES[f + F.q_ly3] * deltat,
                        0);
      ...
      H2Oavail = fmax(POOLS[p + S.D_LF_LY3] * POOLS[p + S.H2O_LY3] -
                          FLUXES[f + F.q_ly3] * deltat,
                      0);
      // Maximum transfer flux in mm (actual transfer may be less due to water
      // or space availability)
    ...
    POOLS[nxp + S.H2O_LY1] =
        POOLS[p + S.H2O_LY1] +
        (FLUXES[f + F.infil] - FLUXES[f + F.ly1xly2] - FLUXES[f + F.q_ly1] -
         FLUXES[f + F.evap] - FLUXES[f + F.transp1]) *
            deltat;
    POOLS[nxp + S.H2O_LY2] =
        POOLS[p + S.H2O_LY2] + (FLUXES[f + F.ly1xly2] - FLUXES[f + F.ly2xly3] -
                                FLUXES[f + F.q_ly2] - FLUXES[f + F.transp2]) *
                                   deltat;
    POOLS[nxp + S.H2O_LY3] =
        POOLS[p + S.H2O_LY3] +
        (FLUXES[f + F.ly2xly3] - FLUXES[f + F.q_ly3]) * deltat;

    if (POOLS[nxp + S.H2O_LY1] > LY1max) {
      // Dump excess into LY1 Q
      FLUXES[f + F.q_ly1] +=
          (POOLS[nxp + S.H2O_LY1] - LY1max) * one_over_deltat;
    ...
    if (POOLS[nxp + S.H2O_LY2] > LY2max) {
      ...
      FLUXES[f + F.q_ly1] +=
          (POOLS[nxp + S.H2O_LY2] - LY2max) * one_over_deltat;
    ...
    if (POOLS[nxp + S.H2O_LY3] > LY3max) {
      ...
      FLUXES[f + F.q_ly3] +=
          (POOLS[nxp + S.H2O_LY3] - LY3max) * one_over_deltat;
      POOLS[nxp + S.H2O_LY3] = LY3max;
    }

    //**********INTERNAL ENERGT FLUXES FOR ALL H2O FLUXES***************
    // Add INFILTRATION, LY1, LY3, LY1xLY3, ET
    double infiltemp = air_temp_k; // Infiltemp needs to be in degrees celcius
                                   // for IF statement to work
    if (FLUXES[f + F.melt] > 0) {
      infiltemp = (infiltemp - DGCM_TK0C) * (PREC[n] - SNOWFALL[n]) /
                      (PREC[n] - SNOWFALL[n] + FLUXES[f + F.melt]) +
                  DGCM_TK0C;
    } // snowmelt temp = 0, so term multiplied by zero in weighted average

    // All energy fluxes

    FLUXES[f + F.infil_e] = FLUXES[f + F.infil] *
                            INTERNAL_ENERGY_PER_LIQUID_H2O_UNIT_MASS(infiltemp);
    FLUXES[f + F.evap_e] =
        FLUXES[f + F.evap] *
        INTERNAL_ENERGY_PER_LIQUID_H2O_UNIT_MASS(POOLS[p + S.D_TEMP_LY1]);
    FLUXES[f + F.transp1_e] =
        FLUXES[f + F.transp1] *
        INTERNAL_ENERGY_PER_LIQUID_H2O_UNIT_MASS(POOLS[p + S.D_TEMP_LY1]);
    FLUXES[f + F.transp2_e] =
        FLUXES[f + F.transp2] *
        INTERNAL_ENERGY_PER_LIQUID_H2O_UNIT_MASS(POOLS[p + S.D_TEMP_LY2]);
    FLUXES[f + F.ly1xly2_e] =
        FLUXES[f + F.ly1xly2] *
        INTERNAL_ENERGY_PER_LIQUID_H2O_UNIT_MASS(TEMPxfer_1to2);
    FLUXES[f + F.ly2xly3_e] =
        FLUXES[f + F.ly2xly3] *
        INTERNAL_ENERGY_PER_LIQUID_H2O_UNIT_MASS(TEMPxfer_2to3);
    FLUXES[f + F.q_ly1_e] =
        FLUXES[f + F.q_ly1] *
        INTERNAL_ENERGY_PER_LIQUID_H2O_UNIT_MASS(POOLS[p + S.D_TEMP_LY1]);
    FLUXES[f + F.q_ly2_e] =
    ...
    FLUXES[f + F.q_ly3_e] =
        FLUXES[f + F.q_ly3] *
        INTERNAL_ENERGY_PER_LIQUID_H2O_UNIT_MASS(POOLS[p + S.D_TEMP_LY3]);
    // Thermal conductivity = k*dT/dz, units are W/m2, converting to J/m2/d
    FLUXES[f + F.ly1xly2_th_e] =
        2 * pars[P.thermal_cond] *
        (POOLS[p + S.D_TEMP_LY1] - POOLS[p + S.D_TEMP_LY2]) /
        (pars[P.LY1_z] + pars[P.LY2_z]) * DGCM_SEC_DAY;
    FLUXES[f + F.ly2xly3_th_e] =
        2 * pars[P.thermal_cond] *
        (POOLS[p + S.D_TEMP_LY2] - POOLS[p + S.D_TEMP_LY3]) /
    ...
    // double frac_ly1 =
    // POOLS[nxp+S.H2O_LY1]/(POOLS[nxp+S.H2O_LY1]+POOLS[nxp+S.H2O_LY3]);
    //  E_LY1

    FLUXES[f + F.geological] = PREDERIVED_GEO_FLUX; // In J/m2/d //105mW/m2
    POOLS[nxp + S.E_LY1] =
        POOLS[p + S.E_LY1] +
        (FLUXES[f + F.gh_in] + FLUXES[f + F.infil_e] - FLUXES[f + F.evap_e] -
         FLUXES[f + F.transp1_e] - FLUXES[f + F.q_ly1_e] -
         FLUXES[f + F.ly1xly2_e] - FLUXES[f + F.ly1xly2_th_e]) *
            deltat;
    POOLS[nxp + S.E_LY2] =
        POOLS[p + S.E_LY2] +
        (FLUXES[f + F.ly1xly2_e] + FLUXES[f + F.ly1xly2_th_e] -
         FLUXES[f + F.transp2_e] - FLUXES[f + F.q_ly2_e] -
         FLUXES[f + F.ly2xly3_e] - FLUXES[f + F.ly2xly3_th_e]) *
            deltat;
    POOLS[nxp + S.E_LY3] =
        POOLS[p + S.E_LY3] +
        (FLUXES[f + F.ly2xly3_e] - FLUXES[f + F.q_ly3_e] +
         FLUXES[f + F.ly2xly3_th_e] + FLUXES[f + F.geological]) *
            deltat;
    ...
    KNORR.IN.DOY = DOY[n];
    KNORR.IN.lambda = LAI;
    KNORR.IN.pasm = (POOLS[p + S.H2O_LY1] + POOLS[nxp + S.H2O_LY1] +
                     POOLS[p + S.H2O_LY2] + POOLS[nxp + S.H2O_LY2]) *
                    0.5; // Note: soil moisture also available here
    ...
    // KNORR.OUT.dlambdadt is in units per timestep; converting thest to units
    // per day (as required for CARDAMOM) "FLUXES" have to be in "per day" units
    ...
    ARFLUXES.IN.ALLOC_FOL_POT = fmax(
        0, ((FLUXES[f + F.target_LAI] * pars[P.LCMA]) - POOLS[p + S.C_fol]) *
               one_over_deltat);
    ARFLUXES.IN.ALLOC_ROO_POT =
        fmax(0, (pars[P.phi_RL] * (FLUXES[f + F.target_LAI] * pars[P.LCMA])) *
                    one_over_deltat);
    ARFLUXES.IN.ALLOC_WOO_POT =
        fmax(0, (pars[P.phi_WL] * (FLUXES[f + F.target_LAI] * pars[P.LCMA])) *
                    one_over_deltat);
    ...
    if (FLUXES[f + F.dlambda_dt] > 0) {
      FLUXES[f + F.fol2lit] = POOLS[p + S.C_fol] *
                              (1 - pow(1 - pars[P.t_foliar], deltat)) *
                              one_over_deltat;
    } else {
      // FLUXES[f+F.dlambda_dt] is in m2/m2/day
      // LCMA = gC/m2/m2
      FLUXES[f + F.fol2lit] = -FLUXES[f + F.dlambda_dt] * pars[P.LCMA] +
                              POOLS[p + S.C_fol] * pars[P.t_foliar];
    }
    ...
    HRJCR.IN.TEMP = POOLS[p + S.D_TEMP_LY1];
    ; // Input in degrees K
    HRJCR.IN.LF = POOLS[p + S.D_LF_LY1];
    ; // Input in degrees
    ...
    FLUXES[f + F.antr] =
        HRJCR.OUT.anaerobic_tr; // Anaerobic turnover rate scalar
    FLUXES[f + F.an_co2_c_ratio] =
        HRJCR.OUT.anaerobic_co2_c_ratio; // CO2_C_ratio
    FLUXES[f + F.an_ch4_c_ratio] =
        HRJCR.OUT.anaerobic_ch4_c_ratio; // CH4_C_ratio

    // outputformat
    // jcr_o 0-3 fT,fV,fW,fCH4; /*jc*/ /* output from JCR module */
    double ae_loss_cwd =
        POOLS[p + S.C_cwd] * HRJCR.OUT.aerobic_tr * pars[P.t_cwd];
    /* aerobic Rh from coarse woody debris*/
    FLUXES[f + F.ae_rh_cwd] = ae_loss_cwd * (1 - pars[P.tr_cwd2som]);
    double ae_loss_lit =
        POOLS[p + S.C_lit] * HRJCR.OUT.aerobic_tr * pars[P.t_lit];
    /* aerobic Rh from litter*/
    FLUXES[f + F.ae_rh_lit] = ae_loss_lit * (1 - pars[P.tr_lit2som]);
    /* aerobic Rh from SOM*/
    FLUXES[f + F.ae_rh_som] =
        POOLS[p + S.C_som] * HRJCR.OUT.aerobic_tr * pars[P.t_som];

    //******Anaerobic fluxes
    double an_loss_cwd =
        POOLS[p + S.C_cwd] * HRJCR.OUT.anaerobic_tr * pars[P.t_cwd];
    /* anaerobic Rh from coarse woody debris*/
    FLUXES[f + F.an_rh_cwd] = an_loss_cwd * (1 - pars[P.tr_cwd2som]);
    /* anaerobic Rh from litter*/
    double an_loss_lit =
        POOLS[p + S.C_lit] * HRJCR.OUT.anaerobic_tr * pars[P.t_lit];
    FLUXES[f + F.an_rh_lit] = an_loss_lit * (1 - pars[P.tr_lit2som]);
    /* anaerobic Rh from SOM*/
    FLUXES[f + F.an_rh_som] =
        POOLS[p + S.C_som] * HRJCR.OUT.anaerobic_tr * pars[P.t_som];
    /*CWD to SOM*/
    FLUXES[f + F.cwd2som] = (an_loss_cwd + ae_loss_cwd) * pars[P.tr_cwd2som];
    /*litter to SOM*/
    FLUXES[f + F.lit2som] = (an_loss_lit + ae_loss_lit) * pars[P.tr_lit2som];
    /* Rh_CO2*/
    FLUXES[f + F.rh_co2] = (FLUXES[f + F.an_rh_lit] + FLUXES[f + F.an_rh_cwd] +
                            FLUXES[f + F.an_rh_som]) *
                               HRJCR.OUT.anaerobic_co2_c_ratio +
                           (FLUXES[f + F.ae_rh_lit] + FLUXES[f + F.ae_rh_cwd] +
                            FLUXES[f + F.ae_rh_som]);
    /* Rh_CH4*/
    FLUXES[f + F.rh_ch4] = (FLUXES[f + F.an_rh_lit] + FLUXES[f + F.an_rh_cwd] +
                            FLUXES[f + F.an_rh_som]) *
                           HRJCR.OUT.anaerobic_ch4_c_ratio;
    ...
    POOLS[nxp + S.C_lab] =
        POOLS[p + S.C_lab] +
        (FLUXES[f + F.gpp] - FLUXES[f + F.resp_auto_maint] -
         FLUXES[f + F.foliar_prod] - FLUXES[f + F.root_prod] -
         FLUXES[f + F.wood_prod] - FLUXES[f + F.resp_auto_growth]) *
            deltat;
    POOLS[nxp + S.C_fol] =
        POOLS[p + S.C_fol] +
        (FLUXES[f + F.foliar_prod] - FLUXES[f + F.fol2lit]) * deltat;
    POOLS[nxp + S.C_roo] =
        POOLS[p + S.C_roo] +
        (FLUXES[f + F.root_prod] - FLUXES[f + F.roo2lit]) * deltat;
    POOLS[nxp + S.C_woo] =
        POOLS[p + S.C_woo] +
        (FLUXES[f + F.wood_prod] - FLUXES[f + F.woo2cwd]) * deltat;
    POOLS[nxp + S.C_cwd] =
        POOLS[p + S.C_cwd] + (FLUXES[f + F.woo2cwd] - FLUXES[f + F.ae_rh_cwd] -
                              FLUXES[f + F.an_rh_cwd] - FLUXES[f + F.cwd2som]) *
                                 deltat;
    POOLS[nxp + S.C_lit] =
        POOLS[p + S.C_lit] + (FLUXES[f + F.fol2lit] + FLUXES[f + F.roo2lit] -
                              FLUXES[f + F.ae_rh_lit] -
                              FLUXES[f + F.an_rh_lit] - FLUXES[f + F.lit2som]) *
                                 deltat;
    POOLS[nxp + S.C_som] =
        POOLS[p + S.C_som] + (FLUXES[f + F.lit2som] - FLUXES[f + F.ae_rh_som] -
                              FLUXES[f + F.an_rh_som] + FLUXES[f + F.cwd2som]) *
                                 deltat;

    // Energy ppols
    // Root zone
    // POOLS[nxp+S.E_LY1] = POOLS[p+S.E_LY1] + (Renato's ground heat flux, inc
    // LWup, etc.)*deltaT  - Runoff terms - Marcos' evaporation terms + Precip
    // energ

    // POOLS[nxp+S.E_LY3] = POOLS[p+S.E_LY1] + (Renato's ground heat
    // flux)*deltaT
    ...
    FLUXES[f + F.f_lab] =
        POOLS[nxp + S.C_lab] * BURNED_AREA[n] * CF[S.C_lab] * one_over_deltat;
    FLUXES[f + F.f_fol] =
        POOLS[nxp + S.C_fol] * BURNED_AREA[n] * CF[S.C_fol] * one_over_deltat;
    FLUXES[f + F.f_roo] =
        POOLS[nxp + S.C_roo] * BURNED_AREA[n] * CF[S.C_roo] * one_over_deltat;
    FLUXES[f + F.f_woo] =
        POOLS[nxp + S.C_woo] * BURNED_AREA[n] * CF[S.C_woo] * one_over_deltat;
    FLUXES[f + F.f_cwd] =
        POOLS[nxp + S.C_cwd] * BURNED_AREA[n] * CF[S.C_cwd] * one_over_deltat;
    FLUXES[f + F.f_lit] =
        POOLS[nxp + S.C_lit] * BURNED_AREA[n] * CF[S.C_lit] * one_over_deltat;
    FLUXES[f + F.f_som] =
        POOLS[nxp + S.C_som] * BURNED_AREA[n] * CF[S.C_som] * one_over_deltat;

    // P*M + P*(1-M)*BAf = P*M + P*BAf - P*M*BAf = P*(M + BAf - M*BAf)  =
    // P*(BAf*(1 - M) + M)
    ...
    FLUXES[f + F.fx_lab2lit] =
        POOLS[nxp + S.C_lab] *
        (NONLEAF_MORTALITY_FACTOR + (1 - LEAF_MORTALITY_FACTOR) *
                                        BURNED_AREA[n] * (1 - CF[S.C_lab]) *
                                        (1 - pars[P.resilience])) *
        one_over_deltat;
    FLUXES[f + F.fx_fol2lit] =
        POOLS[nxp + S.C_fol] *
        (LEAF_MORTALITY_FACTOR + (1 - LEAF_MORTALITY_FACTOR) * BURNED_AREA[n] *
                                     (1 - CF[S.C_fol]) *
                                     (1 - pars[P.resilience])) *
        one_over_deltat;
    FLUXES[f + F.fx_roo2lit] =
        POOLS[nxp + S.C_roo] *
        (NONLEAF_MORTALITY_FACTOR + (1 - NONLEAF_MORTALITY_FACTOR) *
                                        BURNED_AREA[n] * (1 - CF[S.C_roo]) *
                                        (1 - pars[P.resilience])) *
        one_over_deltat;
    FLUXES[f + F.fx_woo2cwd] =
        POOLS[nxp + S.C_woo] *
        (NONLEAF_MORTALITY_FACTOR + (1 - NONLEAF_MORTALITY_FACTOR) *
                                        BURNED_AREA[n] * (1 - CF[S.C_woo]) *
                                        (1 - pars[P.resilience])) *
        one_over_deltat;
    // No mortality in these pools
    FLUXES[f + F.fx_cwd2som] = POOLS[nxp + S.C_cwd] * BURNED_AREA[n] *
                               (1 - CF[S.C_cwd]) * (1 - pars[P.resilience]) *
                               one_over_deltat;
    FLUXES[f + F.fx_lit2som] = POOLS[nxp + S.C_lit] * BURNED_AREA[n] *
                               (1 - CF[S.C_lit]) * (1 - pars[P.resilience]) *
                               one_over_deltat;
    ...
    POOLS[nxp + S.C_lab] =
        POOLS[nxp + S.C_lab] -
        (FLUXES[f + F.f_lab] + FLUXES[f + F.fx_lab2lit]) * deltat;
    POOLS[nxp + S.C_fol] =
        POOLS[nxp + S.C_fol] -
        (FLUXES[f + F.f_fol] + FLUXES[f + F.fx_fol2lit]) * deltat;
    POOLS[nxp + S.C_roo] =
        POOLS[nxp + S.C_roo] -
        (FLUXES[f + F.f_roo] + FLUXES[f + F.fx_roo2lit]) * deltat;
    POOLS[nxp + S.C_woo] =
        POOLS[nxp + S.C_woo] -
        (FLUXES[f + F.f_woo] + FLUXES[f + F.fx_woo2cwd]) * deltat;
    /*dead C pools*/
    /*CWD*/
    POOLS[nxp + S.C_cwd] =
        POOLS[nxp + S.C_cwd] + (FLUXES[f + F.fx_woo2cwd] - FLUXES[f + F.f_cwd] -
                                FLUXES[f + F.fx_cwd2som]) *
                                   deltat;
    /*litter*/
    POOLS[nxp + S.C_lit] =
        POOLS[nxp + S.C_lit] +
        (FLUXES[f + F.fx_lab2lit] + FLUXES[f + F.fx_fol2lit] +
         FLUXES[f + F.fx_roo2lit] - FLUXES[f + F.f_lit] -
         FLUXES[f + F.fx_lit2som]) *
            deltat;
    /*som*/
    POOLS[nxp + S.C_som] = POOLS[nxp + S.C_som] +
                           (FLUXES[f + F.fx_cwd2som] +
                            FLUXES[f + F.fx_lit2som] - FLUXES[f + F.f_som]) *
                               deltat;

    /*fires - total flux in gC m-2 day-1*/
    /*this term is now (essentially) obsolete*/
    /*replace in next version of DALEC_FIRES*/
    FLUXES[f + F.f_total] = FLUXES[f + F.f_lab] + FLUXES[f + F.f_fol] +
                            FLUXES[f + F.f_roo] + FLUXES[f + F.f_woo] +
                            FLUXES[f + F.f_cwd] + FLUXES[f + F.f_lit] +
                            FLUXES[f + F.f_som];

    /*Fraction of C-foliar lost due to fires*/
    FLUXES[f + F.foliar_fire_frac] =
        BURNED_AREA[n] *
        (CF[S.C_lab] + (1 - CF[S.C_lab]) * (1 - pars[P.resilience]));
    /*Calculate LAI (lambda) lost due to fire
      - we lose the same fraction of LAI as we do C-foliar
      - FE_\Lambda^{(t+1)} = \Lambda^{(t+1)'} * BA ( k_{factor(i)} + (1 -
      k_{factor(i)}) r )*/
    FLUXES[f + F.lai_fire] =
        (POOLS[p + S.C_fol] / pars[P.LCMA]) * BURNED_AREA[n] *
        (CF[S.C_lab] + (1 - CF[S.C_lab]) * (1 - pars[P.resilience]));

    /****************************RECORD t+1 DIAGNOSTIC
     * STATES*************************/
    POOLS[nxp + S.D_LAI] = POOLS[nxp + S.C_fol] / pars[P.LCMA]; // LAI
    POOLS[nxp + S.D_SCF] =
        POOLS[nxp + S.H2O_SWE] /
        (POOLS[nxp + S.H2O_SWE] + pars[P.scf_scalar]); // snow cover fraction
    ...
    POOLS[nxp + S.D_SM_LY1] =
        HYDROFUN_EWT2MOI(POOLS[nxp + S.H2O_LY1], pars[P.LY1_por],
                         pars[P.LY1_z]); // soil moisture LY1
    POOLS[nxp + S.D_SM_LY2] =
        HYDROFUN_EWT2MOI(POOLS[nxp + S.H2O_LY2], pars[P.LY2_por],
                         pars[P.LY2_z]); // soil moisture LY2
    POOLS[nxp + S.D_SM_LY3] =
        HYDROFUN_EWT2MOI(POOLS[nxp + S.H2O_LY3], pars[P.LY3_por],
                         pars[P.LY3_z]); // soil moisture LY3

    POOLS[nxp + S.D_PSI_LY1] =
        fmax(HYDROFUN_MOI2PSI(POOLS[nxp + S.D_SM_LY1], psi_porosity,
                              pars[P.retention]),
             minpsi);
    POOLS[nxp + S.D_PSI_LY2] =
        fmax(HYDROFUN_MOI2PSI(POOLS[nxp + S.D_SM_LY2], psi_porosity,
                              pars[P.retention]),
             minpsi);
    POOLS[nxp + S.D_PSI_LY3] =
        fmax(HYDROFUN_MOI2PSI(POOLS[nxp + S.D_SM_LY3], psi_porosity,
                              pars[P.retention]),
             minpsi);

    //     //Isfinite check for 14 progronstic pools only
    //     int nnn, isfinitecheck=1;
    //     for (nnn=0;nnn<14;nnn++){if (
    //     isfinite(POOLS[nxp+nnn])==false){isfinitecheck=0;}}; if
    //     (isfinitecheck==0){break;};
  }
...
int DALEC_1100_MODCONFIG(DALEC *DALECmodel, DATA *DATA) {
  ...
  DALECmodel->nomet =
      10; /*This should be compatible with CBF file, if not then disp error*/
  ...
  // This is generic EDCs structure defined in
  // ../DALEC_EDCs/DALEC_EDC_FUNCTIONS.c Has three args, data (void), function
  // (in "DATA", and "void *", and "double" out), and "boolean" prerun.
  ...
  static DALEC_EDC_MEAN_TEMP_STRUCT EDC_mean_ly1_temp, EDC_mean_ly2_temp,
      EDC_mean_ly3_temp;
  ...
  // declaring observation operator structure, and filling with DALEC
  // configurations
  static OBSOPE OBSOPE;
  // Initialize all SUPPORT OBS values (default value = false).
  INITIALIZE_OBSOPE_SUPPORT(&OBSOPE);

  // Set SUPPORT_OBS values to true if model supports external observation
  // operations.
  ...
  // Note: each OBS operator requirements are unique, see individual observation
  // operator functions to see what's required Note: no values required for any
  // SUPPORT_*_OBS quantity set to false.
