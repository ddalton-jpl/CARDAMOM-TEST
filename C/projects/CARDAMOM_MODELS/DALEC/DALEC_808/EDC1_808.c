...
int EDC1_808(double const *pars, DATA DATA, struct EDCDIAGNOSTIC *EDCD) {
  ...
  */
  ...
  if (((EDC == 1 & DIAG == 0) || DIAG == 1 ||
      ...
      (pars[6] < pars[8] * exp(pars[9] * meantemp))) {
    EDC = 0;
    EDCD->PASSFAIL[4 - 1] = 0;
  }
  ...
  /*if (((EDC==1 & DIAG==0) || DIAG==1) & (fabs(pars[14] - pars[11])<45 |
   * fabs(pars[14]-pars[11])>320.25)){EDC=0;EDCD->PASSFAIL[6-1]=0;}*/
  /*now obsolete!*/
  ...
  if (((EDC == 1 & DIAG == 0) || DIAG == 1 ||
      ...
      (pars[27] < pars[28] | pars[27] < pars[29])) {
    EDC = 0;
   ...
   * EDCD->SWITCH[16-1]==1)) &
   * (pars[24]<pars[25])){EDC=0;EDCD->PASSFAIL[16-1]=0;}
   */

  /*Band checks here*/
  /*if (((EDC==1 & DIAG==0) || DIAG==1 || (EDC==1 & DIAG==2 &
   * EDCD->SWITCH[16-1]==1)) &
   * (pars[33]>pars[34])){EDC=0;EDCD->PASSFAIL[16-1]=0;}*/
  ...
  /*
  if (((EDC==1 & DIAG==0) || DIAG==1 || (EDC==1 & DIAG==2 &
  EDCD->SWITCH[16-1]==1)) & (pars[33]>pars[34])){EDC=0;EDCD->PASSFAIL[16-1]=0;}
  */

  if ((EDC == 1 & DIAG == 0) || DIAG == 1 ||
      (EDC == 1 & DIAG == 2 & EDCD->SWITCH[16 - 1] == 1)) {

    if (Pband_h2o_ns / Pband_h2o_rz > Lband_h2o_ns / Lband_h2o_rz) {
      allbandpass = 0;
    }
    if (Pband_h2o_ns / Pband_woo > Lband_h2o_ns / Lband_woo) {
      allbandpass = 0;
    }
    if (Lband_h2o_ns / Lband_woo > Cband_h2o_ns / Cband_woo) {
      allbandpass = 0;
    }
    if (Lband_fol / Lband_woo > Cband_fol / Cband_woo) {
      allbandpass = 0;
    }
