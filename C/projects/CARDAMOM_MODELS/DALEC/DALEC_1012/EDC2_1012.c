...
int EDC2_1012(double const *pars, DATA DATA, struct EDCDIAGNOSTIC *EDCD) {
  ...
  */
  ...
  Fout[S.C_lit] =
      FT[F.resp_het_lit] + FT[F.lit2som] + FT[F.f_lit] + FT[F.fx_lit2som];
  /*som*/
  Fin[S.C_som] =
      FT[F.wood2lit] + FT[F.lit2som] + FT[F.fx_woo2som] + FT[F.fx_lit2som];
  ...
  if (((EDC == 1 & DIAG == 0) || DIAG == 1 ||
       (EDC == 1 & DIAG == 2 & EDCD->SWITCH[15 - 1] == 1)) &
      (pars[P.wilting] > MPOOLS[S.H2O_PAW])) {
    EDC = ipow(0, EDCD->SWITCH[15 - 1]);
    EDCD->PASSFAIL[15 - 1] = 0;
  }

  // /*EDC jc shuang add changes: PAW < PAW_fs  jc  */
  // if (((EDC==1 & DIAG==0) || DIAG==1 || (EDC==1 & DIAG==2 &
  // EDCD->SWITCH[18-1]==1)) &
  // (pars[P.PAW_fs]<POOLS[S.H2O_PAW])){EDC=ipow(0,EDCD->SWITCH[18-1]);EDCD->PASSFAIL[18-1]=0;}
  ...
  int pidx[] = {P.i_labile, P.i_foliar, P.i_root, P.i_wood,
                P.i_lit,    P.i_som,    P.i_PAW,  P.i_PUW};
