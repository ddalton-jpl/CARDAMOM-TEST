...
int EDC1_1012(double const *pars, DATA DATA, struct EDCDIAGNOSTIC *EDCD) {
  ...
  */
  ...
  /*fraction of GPP som under equilibrium conditions shuang jc change soil to
   * som*/
  double const fsom = fwood + (froot + flab + ffol) * pars[P.tr_lit2som] /
                                  (pars[P.tr_lit2som] + pars[P.t_lit]);
  ...
  if (((EDC == 1 & DIAG == 0) || DIAG == 1 ||
      ...
      (pars[P.tr_lit2som] < pars[P.t_som])) {
    EDC = 0;
    EDCD->PASSFAIL[2 - 1] = 0;
  }
  ...
  if (((EDC == 1 & DIAG == 0) || DIAG == 1 ||
       (EDC == 1 & DIAG == 2 & EDCD->SWITCH[3 - 1] == 1)) &
      (pars[P.t_wood] > torfol)) {
    EDC = 0;
    EDCD->PASSFAIL[3 - 1] = 0;
  }
  ...
  // EDCD->SWITCH[4-1]==1)) &
  // (pars[P.t_root]<pars[P.t_som]*exp(pars[P.temp_factor]*meantemp))){EDC=0;EDCD->PASSFAIL[4-1]=0;}
  if (((EDC == 1 & DIAG == 0) || DIAG == 1 ||
       (EDC == 1 & DIAG == 2 & EDCD->SWITCH[4 - 1] == 1)) &
      (pars[P.t_root] < pars[P.t_som] * pow(pars[P.Q10rhco2], meantemp / 10))) {
    EDC = 0;
    EDCD->PASSFAIL[4 - 1] = 0;
  }
  ...
  if (((EDC == 1 & DIAG == 0) || DIAG == 1 ||
       (EDC == 1 & DIAG == 2 & EDCD->SWITCH[5 - 1] == 1)) &
