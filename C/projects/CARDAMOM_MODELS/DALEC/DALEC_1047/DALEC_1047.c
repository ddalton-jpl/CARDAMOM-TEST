...
int DALEC_1047(DATA DATA, double const *pars) {
  ...
  for (n = 0; n < N_timesteps; n++) {
    ...
    FLUXES[f + F.f_lab] = (POOLS[nxp + S.C_lab] * BURNED_AREA[n] * CF[S.C_lab] +
                           FLUXES[f + F.a_lab]) /
                          deltat;
    FLUXES[f + F.f_fol] = (POOLS[nxp + S.C_fol] * BURNED_AREA[n] * CF[S.C_fol] +
                           FLUXES[f + F.a_fol]) /
                          deltat;
    FLUXES[f + F.f_roo] = (POOLS[nxp + S.C_roo] * BURNED_AREA[n] * CF[S.C_roo] +
                           FLUXES[f + F.a_roo]) /
                          deltat;
    FLUXES[f + F.f_woo] = (POOLS[nxp + S.C_woo] * BURNED_AREA[n] * CF[S.C_woo] +
                           FLUXES[f + F.a_woo]) /
                          deltat;
    FLUXES[f + F.f_lit] =
