...
int EDC2_1017(double const *pars, DATA DATA, struct EDCDIAGNOSTIC *EDCD) {
  ...
  */
  ...
  for (n = 0; n < nopools; n++) {
    ...
    // if (((EDC==1 & DIAG==0) || DIAG==1 || (EDC==1 & DIAG==2 &
    // EDCD->SWITCH[7-1+n]==1)) & ((fabs(log(Rs))>log(EQF)) ||
    // (fabs(Rs-Rm)>etol)))
  ...
  // if (((EDC==1 & DIAG==0) || DIAG==1 || (EDC==1 & DIAG==2 &
  // EDCD->SWITCH[15-1]==1)) &
  // (pars[P.wilting]>MPOOLS[S.H2O_PAW])){EDC=ipow(0,EDCD->SWITCH[15-1]);EDCD->PASSFAIL[15-1]=0;}
  ...
  int pidx[] = {P.i_labile, P.i_foliar, P.i_root, P.i_wood, P.i_lit,
                P.i_soil,   P.i_PAW,    P.i_PUW,  P.i_SWE};
