...
int DALEC_1000(DATA DATA, double const *pars) {
  ...
  */
  ...
  for (n = 0; n < N_timesteps; n++) {
    ...
    /* x (1 + a* P/P0)/(1+a)*/
    FLUXES[f + 1] = exp(pars[9] * 0.5 * (T2M_MIN[n] + T2M_MAX[n] - meantemp)) *
                    ((PREC[n] / meanprec - 1) * pars[32] + 1);
    /*respiration auto*/
    ...
    FLUXES[f + 8] =
        (2 / sqrt(pi)) * (ff / wf) *
        exp(-pow(sin((TIME_INDEX[n] - pars[14] + osf) / sf) * sf / wf, 2));
    /*Labrelease factor*/
    FLUXES[f + 15] =
        (2 / sqrt(pi)) * (fl / wl) *
        exp(-pow(sin((TIME_INDEX[n] - pars[11] + osl) / sf) * sf / wl, 2));
    ...
    POOLS[nxp + 4] = POOLS[p + 4] + (FLUXES[f + 9] + FLUXES[f + 11] -
                                     FLUXES[f + 12] - FLUXES[f + 14]) *
                                        deltat;
    POOLS[nxp + 5] =
        POOLS[p + 5] +
        (FLUXES[f + 14] - FLUXES[f + 13] + FLUXES[f + 10]) * deltat;
    ...
    POOLS[nxp + 6] =
        POOLS[p + 6] +
        (-FLUXES[f + 29] - FLUXES[f + 30] + PREC[n] - FLUXES[f + 28]) * deltat;
    ...
    for (nn = 0; nn < 6; nn++) {
      FLUXES[f + 17 + nn] = POOLS[nxp + nn] * BURNED_AREA[n] * CF[nn] / deltat;
    }
    for (nn = 0; nn < 4; nn++) {
      FLUXES[f + 23 + nn] = POOLS[nxp + nn] * BURNED_AREA[n] * (1 - CF[nn]) *
                            (1 - pars[30]) / deltat;
    }
