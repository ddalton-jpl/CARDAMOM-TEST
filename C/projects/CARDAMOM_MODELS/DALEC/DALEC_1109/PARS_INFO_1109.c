 ...
 *    * parmin*/
/*where is it defined?*/
/*For DALEC_FIRES: as all GPP allocation fractions are inter-dependent (sum =
 * 1)*/
...
int PARS_INFO_1109(DALEC *DALECmodel) {
  ...
  /*Autotrophic maintenance respiration coefficient for roots - turnover rate at
   * 25oC (d-1)*/
  parmin[P.rauto_mr_r] = 0.0001;
  parmax[P.rauto_mr_r] = 0.01;

  /*Autotrophic maintenance respiration coefficient for wood - turnover rate at
   * 25oC (d-1)*/
  ...
  /*Growth yield (g C appearing in new biomass per g C used for growth; "growth
   * yield" in Cannell and Thornley 2000)*/
  ...
  /*LMCA*; Kattge et al. 2011*;*Kattge et al., provide a range of 10 400 g m-2,
   * i.e. 5 200 gC m-2*/
  parmin[P.LCMA] = 5;
  parmax[P.LCMA] = 200;
  ...
  /* jc S_fv statistically fitting the fV curves (S1,S2,S3 schemes) with total
   * soil moisture (PAW/PAW_fs)\n jc new name for this par is S_fv, scalar for
   * aerobic volumetric fraction */
  parmin[P.S_fv] = 1;
  parmax[P.S_fv] = 100.0;
  ...
  /*Spatial range of mean temperature at leaf onset (T_r) (degrees C or degrees
   * kelvin)*/
  parmin[P.T_range] = 0.1;
  parmax[P.T_range] = 10.0;

  /*LAI linear growth constant (inverse of model time units; e.g. days-1 or
   * months-1)*/
  parmin[P.plgr] = 0.001;
  parmax[P.plgr] = 0.5;

  /*Inverse of leaf longevity during senescence period (inverse of model time
   * units; e.g. days-1 or months-1)*/
  ...
  /*Target survival time for LAI under water-deficit conditions (days; or same
   * unit as ET and PAW)*/
  parmin[P.tau_W] = 0.1;
  parmax[P.tau_W] = 300;
  ...
  /*initialization of temperature memory at t=0 (in absolute temperature
   * units)*/
  parmin[P.init_T_mem] = 268.14;
  parmax[P.init_T_mem] = 323.15;

  /*initialization of water/structural memory (fractional value of intrinsic
   * maximum LAI)*/
  ...
  /*PUW energy pool (SWdown Wm2* seconds in year for very very rough OOM
   * estimate)*/
  parmin[P.i_PUW_E] = 2e8;
  parmax[P.i_PUW_E] = 2e9;

  /*PSI 50: water potential when soil-plant continum is at 50% hydraulic
   * conductivity (-MPa)*/
  parmin[P.psi_50] = 0.1;
  parmax[P.psi_50] = 30;
  ...
  /*Sublimation rate: mm/day/SCF/kPha/(MJ/m2/d) Biggest = 1/0.5/2/2; Smallest =
   * 1/1/10/10/*/
