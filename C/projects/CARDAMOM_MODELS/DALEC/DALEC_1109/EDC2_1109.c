...
int EDC2_1109(double const *pars, DATA DATA, struct EDCDIAGNOSTIC *EDCD) {
  ...
  */
  ...
  /*if (((EDC==1 & DIAG==0) || DIAG==1 || (EDC==1 & DIAG==2 &
   * EDCD->SWITCH[6-1]==1)) & (MPOOLS[S.C_fol]>MPOOLS[S.C_roo]*5 |
   * MPOOLS[S.C_fol]*5<MPOOLS[S.C_roo])){EDC=ipow(0,EDCD->SWITCH[6-1]);EDCD->PASSFAIL[6-1]=0;
   * }*/
  EDCD->pEDC = EDCD->pEDC -
               0.5 * pow(log(MPOOLS[S.C_fol] / MPOOLS[S.C_roo]) / log(2), 2);
  ...
  Fout[S.C_lab] = FT[F.foliar_prod] + FT[F.root_prod] + FT[F.wood_prod] +
                  FT[F.f_lab] + FT[F.fx_lab2lit];
  /*foliar*/
  ...
  Fout[S.C_cwd] = FT[F.ae_rh_cwd] + FT[F.an_rh_cwd] + FT[F.cwd2som] +
                  FT[F.f_cwd] + FT[F.fx_cwd2som];
  /*litter*/
  Fin[S.C_lit] = FT[F.fol2lit] + FT[F.roo2lit] + FT[F.fx_lab2lit] +
                 FT[F.fx_fol2lit] + FT[F.fx_roo2lit];
  Fout[S.C_lit] = FT[F.ae_rh_lit] + FT[F.an_rh_lit] + FT[F.lit2som] +
                  FT[F.f_lit] + FT[F.fx_lit2som];
  /*som*/
  Fin[S.C_som] =
      FT[F.cwd2som] + FT[F.lit2som] + FT[F.fx_cwd2som] + FT[F.fx_lit2som];
  ...
  for (n = 0; n < noprogpools; n++) {
    ...
    /*if (((EDC==1 & DIAG==0) || DIAG==1 || (EDC==1 & DIAG==2 &
    EDCD->SWITCH[7-1+n]==1)) & ((fabs(log(Rs))>log(EQF)) || (fabs(Rs-Rm)>etol)))
    {EDC=ipow(0,EDCD->SWITCH[7-1+n]);EDCD->PASSFAIL[7-1+n]=0;}*/
    if (Fin[n] > 0) {
      EDCD->pEDC = EDCD->pEDC - 0.5 * pow(log(Rs) / log(EQF), 2) -
                   0.5 * pow((Rs - Rm) / etol, 2);

      /*storing EDCPROB: i.e. the log probability of each EDC based on a
  ...
  /*Ensuring that wilting point is at or below the mean H2O pool EDC14 (No
   * longer applicable)*/
  ...
  int pidx[] = {P.i_labile, P.i_foliar, P.i_root, P.i_wood, P.i_cwd,
                P.i_lit,    P.i_som,    P.i_PAW,  P.i_PUW,  P.i_SWE};

  /*for (n=0;n<nopools-1;n++){if ((EDC==1 || DIAG==1) &
   * ((MPOOLS[n])>parmax[pidx[n]])){EDC=0;EDCD->PASSFAIL[35-1]=0;}}*/
  for (n = 0; n < noprogpools; n++) {
