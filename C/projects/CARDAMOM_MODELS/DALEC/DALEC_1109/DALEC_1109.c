...
int DALEC_1109(DATA DATA, double const *pars) {
  ...
  /*water pools*/
  POOLS[S.H2O_PAW] =
      HYDROFUN_MOI2EWT(pars[P.i_PAW_SM], pars[P.PAW_por], pars[P.PAW_z]);
  POOLS[S.H2O_PUW] =
      HYDROFUN_MOI2EWT(pars[P.i_PUW_SM], pars[P.PUW_por], pars[P.PUW_z]);
  ...
  POOLS[S.D_SM_PAW] = HYDROFUN_EWT2MOI(POOLS[S.H2O_PAW], pars[P.PAW_por],
                                       pars[P.PAW_z]); // soil moisture PAW
  POOLS[S.D_SM_PUW] = HYDROFUN_EWT2MOI(POOLS[S.H2O_PUW], pars[P.PUW_por],
                                       pars[P.PUW_z]); // soil moisture PUW
  // Convert to potential

  POOLS[S.D_PSI_PAW] =
      HYDROFUN_MOI2PSI(POOLS[S.D_SM_PAW], psi_porosity, pars[P.retention]);
  POOLS[S.D_PSI_PUW] =
      HYDROFUN_MOI2PSI(POOLS[S.D_SM_PUW], psi_porosity, pars[P.retention]);

  // Declare stryct
  SOIL_TEMP_AND_LIQUID_FRAC_STRUCT PAWSOILTEMP, PUWSOILTEMP;
  // Populate with run-specific constrants
  // PAW
  PAWSOILTEMP.IN.dry_soil_vol_heat_capacity = pars[P.PAW_vhc];
  ;                                                // J/m3/K
  ...
  // PUW
  PUWSOILTEMP.IN.dry_soil_vol_heat_capacity = pars[P.PUW_vhc];
  ;                                                // J/m3/K
  ...
  for (n = 0; n < N_timesteps; n++) {
    ...
    double alpha =
        asin((sin(pi / 180 * DATA.ncdf_data.LAT) * sin(pi / 180 * DA) +
              cos(pi / 180 * DATA.ncdf_data.LAT) * cos(pi / 180. * DA) *
                  cos(pi / 180 * h))) *
    ...
    // double psi_PAW0 =
    // HYDROFUN_MOI2PSI(max(POOLS[p+S.D_SM_PAW],0),psi_porosity,pars[P.retention]);
    // double psi_PAW0 =
    // HYDROFUN_MOI2PSI(POOLS[p+S.D_SM_PAW],psi_porosity,pars[P.retention_paw]);
    double beta =
        1 / (1 + exp(pars[P.beta_lgr] *
                     (-1 * POOLS[p + S.D_PSI_PAW] / pars[P.psi_50] - 1)));
    ...
    double SNOWMELT =
        fmin(fmax((DGCM_TK0C + SKT[n] - pars[P.min_melt]) * pars[P.melt_slope],
                  0),
             1) *
        POOLS[nxp + S.H2O_SWE] / deltat; /*melted snow per day*/
    ...
    POOLS[nxp + S.H2O_SWE] =
        POOLS[nxp + S.H2O_SWE] -
        (FLUXES[f + F.melt] + FLUXES[f + F.sublimation]) *
            deltat; /*second step remove snowmelt from SWE*/
    ...
    FLUXES[f + F.ground_heat] =
        (pars[P.thermal_cond_surf] * (tskin_k - POOLS[p + S.D_TEMP_PAW]) /
         (pars[P.PAW_z] * 0.5)) *
        (1. - POOLS[p + S.D_SCF]);
    ...
    // double sm_PAW =
    // HYDROFUN_EWT2MOI(POOLS[p+S.H2O_PAW],pars[P.PAW_por],pars[P.PAW_z]);
    // double sm_PUW =
    // HYDROFUN_EWT2MOI(POOLS[p+S.H2O_PUW],pars[P.PUW_por],pars[P.PUW_z]);
    ...
    double drain_PAW =
        POOLS[p + S.D_LF_PAW] * DRAINAGE(POOLS[p + S.D_SM_PAW],
                                         pars[P.Q_excess], -pars[P.field_cap],
                                         psi_porosity, pars[P.retention]);
    double drain_PUW =
        POOLS[p + S.D_LF_PUW] * DRAINAGE(POOLS[p + S.D_SM_PUW],
                                         pars[P.Q_excess], -pars[P.field_cap],
                                         psi_porosity, pars[P.retention]);

    // Drainage becomes runoff from pools
    FLUXES[f + F.q_paw] =
        HYDROFUN_MOI2EWT(drain_PAW, pars[P.PAW_por], pars[P.PAW_z]) / deltat;
    FLUXES[f + F.q_puw] =
        HYDROFUN_MOI2EWT(drain_PUW, pars[P.PUW_por], pars[P.PUW_z]) / deltat;
    ...
    double k_PAW = HYDROFUN_MOI2CON(POOLS[p + S.D_SM_PAW], pars[P.hydr_cond],
                                    pars[P.retention]);
    double k_PUW = HYDROFUN_MOI2CON(POOLS[p + S.D_SM_PUW], pars[P.hydr_cond],
                                    pars[P.retention]);

    // // Convert to potential
    // double psi_PAW =
    // HYDROFUN_MOI2PSI(sm_PAW,psi_porosity,pars[P.retention_paw]); double
    // psi_PUW = HYDROFUN_MOI2PSI(sm_PUW,psi_porosity,pars[P.retention_puw]);

    // Calculate inter-pool transfer in m/s (positive is PAW to PUW)
    double pot_xfer =
        1000 * sqrt(k_PAW * k_PUW) *
        (1000 * (POOLS[p + S.D_PSI_PAW] - POOLS[p + S.D_PSI_PUW]) /
             (9.8 * 0.5 * (pars[P.PAW_z] + pars[P.PUW_z])) +
         1);
    ...
    if (pot_xfer > 0) { // Water is going PAW->PUW (down)
      ...
      SPACEavail = fmax(pars[P.PUW_z] * pars[P.PUW_por] * 1e3 -
                            POOLS[p + S.H2O_PUW] + FLUXES[f + F.q_puw] * deltat,
                        0);
      // Available water in PAW (after runoff, et, and infiltration)
      H2Oavail = fmax(
          POOLS[p + S.D_LF_PAW] * POOLS[p + S.H2O_PAW] +
              (FLUXES[f + F.infil] - FLUXES[f + F.q_paw] - FLUXES[f + F.et]) *
                  deltat,
          0);
      // Maximum transfer flux in mm (actual transfer may be less due to water
      // or space availability)
      PAW2PUWmax =
          POOLS[p + S.D_LF_PAW] * pot_xfer * 1000 * DGCM_SEC_DAY * deltat;
      // Minimum of three terms for PAW->PUW
      // 1. PAW2PUWmax
      // 2. Available space in PUW (after runoff)
      // 3. PAW*LF (after runoff, et, and infiltration)
      FLUXES[f + F.paw2puw] =
          fmin(PAW2PUWmax, fmin(SPACEavail, H2Oavail)) / deltat;
      TEMPxfer = POOLS[p + S.D_TEMP_PAW]; // In K
    } else {                              // Water is going PUW->PAW (up)
      // Available space in PAW after runoff, et, and infiltration
      SPACEavail = fmax(
          pars[P.PAW_z] * pars[P.PAW_por] * 1e3 - POOLS[p + S.H2O_PAW] -
              (FLUXES[f + F.infil] - FLUXES[f + F.q_paw] - FLUXES[f + F.et]) *
                  deltat,
          0);
      // Available water in PUW after runoff
      H2Oavail = fmax(POOLS[p + S.D_LF_PUW] * POOLS[p + S.H2O_PUW] -
                          FLUXES[f + F.q_puw] * deltat,
                      0);
      // Maximum transfer flux in mm (actual transfer may be less due to water
      // or space availability)
      PAW2PUWmax =
          POOLS[p + S.D_LF_PUW] * pot_xfer * 1000 * DGCM_SEC_DAY * deltat;
    ...
    POOLS[nxp + S.H2O_PAW] =
        POOLS[p + S.H2O_PAW] +
        (FLUXES[f + F.infil] - FLUXES[f + F.paw2puw] - FLUXES[f + F.q_paw] -
         FLUXES[f + F.evap] - FLUXES[f + F.transp]) *
            deltat;
    POOLS[nxp + S.H2O_PUW] =
        POOLS[p + S.H2O_PUW] +
        (FLUXES[f + F.paw2puw] - FLUXES[f + F.q_puw]) * deltat;
    ...
    FLUXES[f + F.et_e] =
        (FLUXES[f + F.transp] + FLUXES[f + F.evap]) *
        INTERNAL_ENERGY_PER_LIQUID_H2O_UNIT_MASS(POOLS[p + S.D_TEMP_PAW]);
    FLUXES[f + F.paw2puw_e] =
        FLUXES[f + F.paw2puw] *
        INTERNAL_ENERGY_PER_LIQUID_H2O_UNIT_MASS(TEMPxfer);
    FLUXES[f + F.q_paw_e] =
        FLUXES[f + F.q_paw] *
        INTERNAL_ENERGY_PER_LIQUID_H2O_UNIT_MASS(POOLS[p + S.D_TEMP_PAW]);
    FLUXES[f + F.q_puw_e] =
        FLUXES[f + F.q_puw] *
        INTERNAL_ENERGY_PER_LIQUID_H2O_UNIT_MASS(POOLS[p + S.D_TEMP_PUW]);
    // Thermal conductivity = k*dT/dz, units are W/m2, converting to J/m2/d
    FLUXES[f + F.paw2puw_th_e] =
        2 * pars[P.thermal_cond] *
        (POOLS[p + S.D_TEMP_PAW] - POOLS[p + S.D_TEMP_PUW]) /
        (pars[P.PAW_z] + pars[P.PUW_z]) * DGCM_SEC_DAY;
    ...
    // double frac_paw =
    // POOLS[nxp+S.H2O_PAW]/(POOLS[nxp+S.H2O_PAW]+POOLS[nxp+S.H2O_PUW]);
    //  E_PAW

    FLUXES[f + F.geological] = 0.105 * 3600 * 24; // In J/m2/d //105mW/m2
    POOLS[nxp + S.E_PAW] =
        POOLS[p + S.E_PAW] +
        (FLUXES[f + F.gh_in] + FLUXES[f + F.infil_e] - FLUXES[f + F.et_e] -
         FLUXES[f + F.q_paw_e] - FLUXES[f + F.paw2puw_e] -
         FLUXES[f + F.paw2puw_th_e]) *
            deltat;
    POOLS[nxp + S.E_PUW] =
        POOLS[p + S.E_PUW] +
        (FLUXES[f + F.paw2puw_e] - FLUXES[f + F.q_puw_e] +
         FLUXES[f + F.paw2puw_th_e] + FLUXES[f + F.geological]) *
            deltat;
    ...
    KNORR.IN.DOY = DOY[n];
    KNORR.IN.lambda = LAI;
    KNORR.IN.pasm = (POOLS[p + S.H2O_PAW] + POOLS[nxp + S.H2O_PAW]) /
    ...
    if (FLUXES[f + F.dlambda_dt] > 0) {
      FLUXES[f + F.fol2lit] =
          POOLS[p + S.C_fol] * (1 - pow(1 - pars[P.t_foliar], deltat)) / deltat;
    } else {
      // FLUXES[f+F.dlambda_dt] is in m2/m2/day
      // LCMA = gC/m2/m2
      FLUXES[f + F.fol2lit] =
          -FLUXES[f + F.dlambda_dt] * pars[P.LCMA] +
          POOLS[p + S.C_fol] * (1 - pow(1 - pars[P.t_foliar], deltat)) / deltat;
    }
    ...
    /*wood CWD production*/
    FLUXES[f + F.woo2cwd] =
        POOLS[p + S.C_woo] * (1 - pow(1 - pars[P.t_wood], deltat)) / deltat;
    /*root litter production*/
    FLUXES[f + F.roo2lit] =
        POOLS[p + S.C_roo] * (1 - pow(1 - pars[P.t_root], deltat)) / deltat;

    /*-----------------------------------------------------------------------*/

    // TIME-VARYING INPUTS
    HRJCR.IN.SM = POOLS[p + S.D_SM_PAW];
    HRJCR.IN.TEMP = POOLS[p + S.D_TEMP_PAW];
    ; // Input in degrees K
    HRJCR.IN.LF = POOLS[p + S.D_LF_PAW];
    ; // Input in degrees
    ...
    // Energy ppols
    // Root zone
    // POOLS[nxp+S.E_PAW] = POOLS[p+S.E_PAW] + (Renato's ground heat flux, inc
    // LWup, etc.)*deltaT  - Runoff terms - Marcos' evaporation terms + Precip
    // energ

    // POOLS[nxp+S.E_PUW] = POOLS[p+S.E_PAW] + (Renato's ground heat
    // flux)*deltaT
    ...
    FLUXES[f + F.f_lab] =
        POOLS[nxp + S.C_lab] * BURNED_AREA[n] * CF[S.C_lab] / deltat;
    FLUXES[f + F.f_fol] =
        POOLS[nxp + S.C_fol] * BURNED_AREA[n] * CF[S.C_fol] / deltat;
    FLUXES[f + F.f_roo] =
        POOLS[nxp + S.C_roo] * BURNED_AREA[n] * CF[S.C_roo] / deltat;
    FLUXES[f + F.f_woo] =
        POOLS[nxp + S.C_woo] * BURNED_AREA[n] * CF[S.C_woo] / deltat;
    FLUXES[f + F.f_cwd] =
        POOLS[nxp + S.C_cwd] * BURNED_AREA[n] * CF[S.C_cwd] / deltat;
    FLUXES[f + F.f_lit] =
        POOLS[nxp + S.C_lit] * BURNED_AREA[n] * CF[S.C_lit] / deltat;
    FLUXES[f + F.f_som] =
        POOLS[nxp + S.C_som] * BURNED_AREA[n] * CF[S.C_som] / deltat;
    ...
    POOLS[nxp + S.D_SM_PAW] =
        HYDROFUN_EWT2MOI(POOLS[nxp + S.H2O_PAW], pars[P.PAW_por],
                         pars[P.PAW_z]); // soil moisture PAW
    POOLS[nxp + S.D_SM_PUW] =
        HYDROFUN_EWT2MOI(POOLS[nxp + S.H2O_PUW], pars[P.PUW_por],
                         pars[P.PUW_z]); // soil moisture PUW

    POOLS[nxp + S.D_PSI_PAW] = HYDROFUN_MOI2PSI(
        POOLS[nxp + S.D_SM_PAW], psi_porosity, pars[P.retention]);
    POOLS[nxp + S.D_PSI_PUW] = HYDROFUN_MOI2PSI(
        POOLS[nxp + S.D_SM_PUW], psi_porosity, pars[P.retention]);
  }
