...
int EDC1_1080(double const *pars, DATA DATA, struct EDCDIAGNOSTIC *EDCD) {
  ...
  */
  ...
  /*EDC1 contains all checks that do not require a full DALEC_CDEA (DALEC2)
   * model run*/
  ...
  for (m = 0; m < 2; m++) {
    ...
    double const fsom =
        fwood + (froot + flab + ffol) * pars[P.tr_lit2soil_1 + m] /
                    (pars[P.tr_lit2soil_1 + m] + pars[P.t_lit_1 + m]);
    ...
    if (((EDC == 1 & DIAG == 0) || DIAG == 1 ||
         (EDC == 1 & DIAG == 2 & EDCD->SWITCH[edco + 1 - 1] == 1)) &
        (pars[P.t_soil_1 + m] > pars[P.t_lit_1 + m])) {
      EDC = 0;
      EDCD->PASSFAIL[edco + 1 - 1] = 0;
    }

    /*EDC CHECK NO 2*/
    /*Litter2SOM greater than SOM to Atm. rate*/
    if (((EDC == 1 & DIAG == 0) || DIAG == 1 ||
         (EDC == 1 & DIAG == 2 & EDCD->SWITCH[edco + 2 - 1] == 1)) &
        (pars[P.tr_lit2soil_1 + m] < pars[P.t_soil_1 + m])) {
      EDC = 0;
      EDCD->PASSFAIL[edco + 2 - 1] = 0;
    }

    /*EDC CHECK NO 3*/
    /*TOR_FOL faster than TOR_WOOD */
    if (((EDC == 1 & DIAG == 0) || DIAG == 1 ||
         (EDC == 1 & DIAG == 2 & EDCD->SWITCH[edco + 3 - 1] == 1)) &
        (pars[P.t_wood_1 + m] > torfol)) {
      EDC = 0;
      EDCD->PASSFAIL[edco + 3 - 1] = 0;
    }
    ...
    if (((EDC == 1 & DIAG == 0) || DIAG == 1 ||
         (EDC == 1 & DIAG == 2 & EDCD->SWITCH[edco + 4 - 1] == 1)) &
        (pars[P.t_root_1 + m] <
         pars[P.t_soil_1 + m] * exp(pars[P.temp_factor_1 + m] * meantemp))) {
      EDC = 0;
      EDCD->PASSFAIL[edco + 4 - 1] = 0;
    }
    ...
    /*if (((EDC==1 & DIAG==0) || DIAG==1) & (fabs(pars[P.Fday] -
     * pars[P.Bday])<45 |
     * fabs(pars[P.Fday]-pars[P.Bday])>320.25)){EDC=0;EDCD->PASSFAIL[edco+6-1]=0;}*/
    /*now obsolete!*/

    /*EDC CHECK NO 5*/
    /*Allocation to canopy is comparable to allocation to fine roots*/
    if (((EDC == 1 & DIAG == 0) || DIAG == 1 ||
         (EDC == 1 & DIAG == 2 & EDCD->SWITCH[edco + 5 - 1] == 1)) &
        ((ffol + flab) > 5 * froot | (ffol + flab) * 5 < froot)) {
      EDC = 0;
      EDCD->PASSFAIL[edco + 5 - 1] = 0;
    }

    /*EDC No 16* Foliage CF> wood CF & SOM CF*/
    if (((EDC == 1 & DIAG == 0) || DIAG == 1 ||
         (EDC == 1 & DIAG == 2 & EDCD->SWITCH[edco + 40 - 1] == 1)) &
        (pars[P.cf_foliar_1 + m] < pars[P.cf_ligneous_1 + m] |
         pars[P.cf_foliar_1 + m] < pars[P.cf_DOM_1 + m])) {
      EDC = 0;
      EDCD->PASSFAIL[edco + 40 - 1] = 0;
    }

    /*EDC No 17: RD_puw > RD_paw*/
    // if (((EDC==1 & DIAG==0) || DIAG==1 || (EDC==1 & DIAG==2 &
    // EDCD->SWITCH[edco+17-1]==1)) &
    // (pars[P.PAW_Qmax]<pars[P.P])){EDC=0;EDCD->PASSFAIL[edco+17-1]=0;}
