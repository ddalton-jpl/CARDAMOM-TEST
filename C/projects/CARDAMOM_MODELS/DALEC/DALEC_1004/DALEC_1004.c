  ...
  if (EDC == 1 | MODEL->EDCD->DIAG > 0) {
    ...
    */
    ...
    for (n = 0; n < nr; n++) {
      ...
      /*GPP*/
      FLUXES[f + 0] =
          ACM(gpppars, constants) * fmin(POOLS[p + 6] / pars[25], 1);
      ...
      /* x (1 + a* P/P0)/(1+a)*/
      FLUXES[f + 1] = exp(pars[9] * 0.5 *
                          (DATA.MET[m + 2] + DATA.MET[m + 1] - DATA.meantemp)) *
      ...
      FLUXES[f + 10] =
          POOLS[p + 3] * (1 - pow(1 - pars[6 - 1], deltat)) / deltat;
      /*root litter production*/
      FLUXES[f + 11] =
          POOLS[p + 2] * (1 - pow(1 - pars[7 - 1], deltat)) / deltat;
      ...
      POOLS[nxp + 1] = POOLS[p + 1] +
                       (FLUXES[f + 3] - FLUXES[f + 9] + FLUXES[f + 7]) * deltat;
      ...
      FLUXES[f + 29] =
          pow(POOLS[p + 6], 2) / pars[24] / deltat * (1 - pars[33]);
      /*PAW -> PUW transfer*/
      ...
      if (POOLS[p + 6] > pars[24] / 2) {
        FLUXES[f + 29] =
            (POOLS[p + 6] - pars[24] / 4) / deltat * (1 - pars[33]);
        FLUXES[f + 30] =
      ...
      POOLS[nxp + 7] =
          POOLS[p + 7] + (FLUXES[f + 30] - FLUXES[f + 31]) * deltat;
      ...
      for (nn = 0; nn < 6; nn++) {
        FLUXES[f + 17 + nn] =
            POOLS[nxp + nn] * DATA.MET[m + 6] * CF[nn] / deltat;
      }
      for (nn = 0; nn < 4; nn++) {
      ...
      POOLS[nxp + 5] =
          POOLS[nxp + 5] +
          (FLUXES[f + 23 + 3] + FLUXES[f + 23 + 4] - FLUXES[f + 17 + 5]) *
              deltat;
      ...
      NEE[n] = -FLUXES[f + 0] + FLUXES[f + 2] + FLUXES[f + 12] +
               FLUXES[f + 13] + FLUXES[f + 16];
    }

    EDC = ipow(EDC2_1004(pars, DATA, MODEL->EDCD), DATA.EDC);
    P = P + log((double)EDC);
    for (k = 0; k < 100; k++) {
      printf("%i ", DATA.M_EDCD[k]);
    }
    printf("\n");
  }
  printf("***");
