#pragma once
/* Functions to support the Leaf Phenology Model of Knorr et al. (2010,
 * doi: 10.1029/2009JG001119)*/
//
// Minimum function with quadratic transition (smoothed) between two values.
// Example: x  = (double)0.1; y  = (double)1e-6; x0 = 5e-3 z =
// MinimumQuadraticSmooth(x, y, x0)
/* x  = first value */
/* y  = second value */
...
double MaxExponentialSmooth(double const x, double const y, double const x0) {
  ...
  if (x >= (y - x0)) {
    maxx = x + x0 * exp(-(x - y) / x0 - 1.0); // statements to be executed if
...
double ComputeDaylightHours(double const *pars) {
  ...
  */
   ...
   *   consts= &consts;
   *   */
  dec = (double)-23.4 * cos((360. * (pars[1] + 10.) / 365.) * pars[2] / 180.) *
        pars[2] / 180.;
