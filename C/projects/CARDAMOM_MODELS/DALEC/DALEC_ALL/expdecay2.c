...
double expdecay2(double const *POOLS, int pool, int nodays, int deltat,
                 int nopools)
/*only accepting pool and number of days, the rest done here*/
{
  ...
  /*offset in days*/
  /*OFFSET = 1 is ideal to capture rapid exponential decay without compromising
   * quality of fit*/
  int os = 1;
  ...
  for (n = 0; n < aw; n++) {
    MP0 = MP0 + POOLS[n * nopools + pool];
  };
  MP0 = MP0 / (double)aw;
  for (n = aw; n < aw * 2; n++) {
    MP1 = MP1 + POOLS[n * nopools + pool];
  };
  MP1 = MP1 / (double)aw;
  for (n = os; n < aw + os; n++) {
    MP0os = MP0os + POOLS[n * nopools + pool];
  };
  MP0os = MP0os / (double)aw;
  for (n = aw + os; n < aw * 2 + os; n++) {
    MP1os = MP1os + POOLS[n * nopools + pool];
  };
  MP1os = MP1os / (double)aw;
  ...
  */
  ...
  /*if (fabs(-log(2)/C)<noyears*365.25 & C<0 & finite(C) &
   * abs(B)>abs(A)*0.01){EDC=0;}*/
  return C;
}
