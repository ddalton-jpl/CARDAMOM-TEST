...
// in the brackets are input fed from main module, use pointer to a double if
// output is an array

double *JCR(double const *ch4pars, double T2M_MIN, double T2M_MAX,
            double thetas) {
  /* input array already defined above,
define names for ch4pars elements to be convenient in the JCR module*/
  double PAW_fs, S_fv, thetas_opt, fwc, r_ch4, Q10ch4, Q10rhco2,
      meantemp; /*jc*/
  ...
  S_fv = ch4pars[0];
  /*optimum water scaler (fW), where the maximum respiration is achieved without
  limitation from water it is the highest point in the function of PAW/PAW_fs
  against water scaler (fW)*/
  ...
  // fCH4 = r_ch4 * fT_ch4;
  /* fmin() won't be needed once CH4 data is assimilated, as CH4 portion of CO2
  should be very small, manually put the upper limit 1 here mainly based on the
  mass balance, but notice that DIC is not considered to contribute when the
  current timestep Rh is depeleted by CH4. I think it's good for
