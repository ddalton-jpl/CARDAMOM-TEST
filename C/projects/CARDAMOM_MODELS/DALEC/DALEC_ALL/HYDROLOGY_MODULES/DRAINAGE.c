...
double DRAINAGE(double sm, double Qexcess, double psi_field,
                double psi_porosity, double b) {
  ...
  // Matlab line Massoud et al.  D= frac_Qexcess * (1-
  // (PARS.psiporosity-min(max(STATES.PSI(t,:),PARS.psifield),PARS.psiporosity))./(PARS.psiporosity-PARS.psifield)).*(SMdelta);
  // calculate drainage (equals zero if change in soil moisture is zero)
  double drainage =
      excess_drainage +
      delta_sm * Qexcess *
          (1 - (psi_porosity - fmin(fmax(psi, psi_field), psi_porosity)) /
