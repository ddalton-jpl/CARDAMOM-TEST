...
typedef struct {
  struct {
    ...
    double TEMP;     // K
    double C_LIVE_W; // Live C in wood only. Note that this includes heartwood
                     // C, and overall maintenance respiration reflects bulk
                     // cost of live + dead wood.
    double C_LIVE_R; // Live C in other pools (other live pools, exluding NSCs
                     // themselves).
    double NSC; // labile carbon pool, Clab, non-structural carbohydrates (gC)
    double GPP;
    double Rd;   // Canopy level dark/day respiration
    double mr_r; // parameter: maintenance respiration coefficient for root
                 // (gC/gC/d) - see Cannell and Thornley (2000,
                 // doi: 10.1006/anbo.1999.0996)
    double mr_w; // parameter: maintenance respiration coefficient for wood
                 // (gC/gC/d) - will be smaller than mr_r from EDC.
    double
        gr; // parameter: growth yield parameter (gC/gC) - see "growth yield"
            // (Y_g) in Cannell and Thornley (2000, doi: 10.1006/anbo.1999.0996)
    double Q10mr; // parameter: Q10 parameter for maintenance respiration
                  // (unitless)
  ...
  struct {
    ...
    double AUTO_RESP_MAINTENANCE; // autotrophic maintenance respiration flux
                                  // (gC/m2/d)
    ...
    double
        NONLEAF_MORTALITY_FACTOR; // Proportional removal from all three pools.
  } OUT;
...
int ALLOC_AND_AUTO_RESP_FLUXES(ALLOC_AND_AUTO_RESP_FLUXES_STRUCT *S) {
  ...
  fT = pow(S->IN.Q10mr, (S->IN.TEMP - (25 + DGCM_TK0C)) /
                            10); // reference temperature is 25 degrees C
  // Autotrophic maintenance for wood and non-wood
  double POTENTIAL_AUTO_RESP_MAINTENANCE =
      S->IN.mr_w * fT * S->IN.C_LIVE_W + S->IN.mr_r * fT * S->IN.C_LIVE_R;

  // Can be negative -- accounting for net Co2 loss in Ag - Rd -- but can't
  // exceed NSC availability
  ...
  if (POTENTIAL_AUTO_RESP_MAINTENANCE > NSC_PLUS_GPP_RATE) {
    ...
    // if NSC_PLUS_GPP_RATE = POTENTIAL_AUTO_RESP_MAINTENANCE; MORTALITY = 0;
    // all biomass is lost.

    // MORTALITY_FACTOR  = 1 -
    // NSC_PLUS_GPP_RATE/POTENTIAL_AUTO_RESP_MAINTENANCE; MORTALITY_FACTOR  = 1
    // - NSC_PLUS_GPP_RATE/POTENTIAL_AUTO_RESP_MAINTENANCE; Current model: if %
    // maintenance resp not available, then lose same % of biomass. Alernative
    // model: remobilize foliar and fine root sugars (if at all possible, check
    // literature)
    S->OUT.NONLEAF_MORTALITY_FACTOR =
        fmin(1 - NSC_PLUS_GPP_RATE / POTENTIAL_AUTO_RESP_MAINTENANCE, 1);
  }
  ...
  if (S->OUT.NONLEAF_MORTALITY_FACTOR == 0) {
    ...
    double LEFTOVER_NSC_RATE =
        NSC_PLUS_GPP_RATE - S->OUT.AUTO_RESP_MAINTENANCE + S->IN.Rd;
    ...
    // NEW PSEUDOCODE: F_LABREL_SUPPLY = fmax(0,  (1 - S->IN.gr) *
    // (LEFTOVER_NSC/ S->IN.deltat));

    // Potential demand of labile carbon by plant growth
    TOTAL_GROWTH_POT =
        S->IN.ALLOC_FOL_POT + S->IN.ALLOC_WOO_POT + S->IN.ALLOC_ROO_POT;
    F_LABREL_DEMAND = fmax(0, TOTAL_GROWTH_POT);
    // Actual release of labile carbon (before growth respiration costs
    // subtracted)
    S->OUT.F_LABREL_ACTUAL = fmin(F_LABREL_SUPPLY, F_LABREL_DEMAND);

    // Scaling factor for allocation fluxes, accounts for NSC limitation and
    // growth respiration cost
    //  - if actual growth is smaller than potential growth, we down-scale plant
    //  allocation fluxes
    ...
    // Actual release of labile carbon i.e. growth flux (after subtracting
    // growth respiration costs)
  ...
  S->OUT.AUTO_RESP_TOTAL =
      S->OUT.AUTO_RESP_MAINTENANCE + S->OUT.AUTO_RESP_GROWTH;
