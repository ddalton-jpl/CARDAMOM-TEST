...
double *LAI_GSI(double const *met_list, double const *var_list) {
  ...
  */
  ...
  // Forcings/drivers
  Tmn = (double)met_list[0]; // Original DALEC model, Tmn is the 21 day average
                             // Tmn, MET[m+9]
  VPD = (double)met_list[1]; // Original DALEC model, this is the 21 day average
                             // VPD, MET[m+11]
  ...
  daylength = ComputeDaylightHours(
      daylengthpars); // Original DALEC model, this is the 21 day average
                      // photoperiod, MET[m+10]

  /* 25/09/14 - JFE
  Here we calculate the Growing Season Index based on
   Jolly et al. A generalized, bioclimatic index to predict foliar phenology in
  response to climate Global Change Biology, Volume 11, page 619-632 - 2005
  (doi: 10.1111/j.1365-2486.2005.00930.x)
  - Stoeckli, R., T. Rutishauser, I. Baker, M. A. Liniger, and A. S. Denning
  (2011), A global reanalysis of vegetation phenology, J. Geophys. Res., 116,
  G03020, doi:10.1029/2010JG001545.
  ...
  min and max value. Temperature, photoperiod and VPD are direct input
  */
  ...
  if (daylength > Photoperiodmax) // photoperiod larger than parameter PHOTOmax
  {
    PHOTOfac = 1.;
             ...
             Photoperiodmin) // photoperiod lower than parameter PHOTOmin
  {
    PHOTOfac = 0.;
  } else {
