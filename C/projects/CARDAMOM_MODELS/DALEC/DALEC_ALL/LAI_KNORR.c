...
double *LAI_KNORR(double const *met_list, double const *var_list) {
  /*Notes: DALEC code passes the following *consts and *pars pointers:
   double
  constants[10]={pars[10],0.0156935,4.22273,208.868,0.0453194,0.37836,7.19298,
  0.011136,2.1001,0.789798};

  constants terms:
  consts[0] = T_Phi = 12.5        # parameter: temperature at leaf onset
  (degrees C) consts[1] = T_r   = 2.0         # parameter: spatial range (1
  sigma) of T_Phi (degrees C) consts[2] = t_c   = 10.0        # parameter: day
  length at leaf shedding (hours) consts[3] = t_r   = 0.5         # parameter:
  spatial range (1 sigma) of t_c (hours) consts[4] = xi  = 0.5           #
  parameter: initial linear leaf growth (1/days) consts[5] = k_L   = 0.1 #
  parameter: inverse of leaf longevity from start of senscence (1/days)
  consts[6] = lambda_max = 6.0.   # parameter: maximum potential leaf area index
  (m2/m2)
  */
  ...
  if (met_list[0] < 100.0) {
    // assume temperature forcing at time t is in degrees C, then convert to
    // degrees K
  ...
  tau_m =
      (double)30.0; // Averaging period for temperature growth trigger T (units
                    // of days; must be in same units as deltat), usually kept
                    // constant but could potentially be a tuneable parameter
  ...
  lambda_max_memory = (double)var_list[11]; // TIME-DEPENDENT I.E. SAVE IN
                                            // MEMORY
  tau_s =
      (double)30.0; // Averaging period for water/structural memory (units of
                    // days; must be in same units as deltat), usually kept
                    // constant but could potentially be a tuneable parameter
  ...
  // if (n==0){T_memory=T_phi+3*T_r;}   /* set the temperature memory to be high
  // so that we start in growth phase */ Exponentially declining memory of
  // temperature
  T = exp(-deltat / tau_m) * T_memory + meantemp * (1 - exp(-deltat / tau_m));
  T_deviation = (T - T_phi) / T_r;
  /* fraction of plants above temperature threshold: using cumulative normal
   * distribution function (derived from the intrinsic c-function erfc) */
  ...
  /* fraction of plants above daylength threshold: using cumulative normal
   * distribution function (derived from the intrinsic c-function erfc) */
  ...
  /* update LAI water/structural memory using an exponentially declining memory
   * of water/structural limitation over the time period tau_s */
  laim = exp(-deltat / tau_s) * lambda_max_memory +
         lambda_tilde_max * (1.0 - exp(-deltat / tau_s));
