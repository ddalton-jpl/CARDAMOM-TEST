...
double *BALDOCCHI_ANALYTICAL(double const *met_list, double const *var_list) {
  ...
  /* Note: Instabilities present if 'm' is <5 or Jmax25/Vmax25 < 0.8 (and vis
   * versa)*/
  /* Note: Additional instabilities for very high values of VPD not generally
   * seen on Earth*/
  ...
  pi_here = (double)3.14;
  dec = (double)-23.4 * cos((360. * (doy + 10.) / 365.) * pi_here / 180.) *
        pi_here / 180.;
  ...
  double par_frac, Wm2_umol, second_per_day, parts_ppm, gram_per_mol_C,
      gram_per_mol_H20, gram_per_kg, diffusionH2OtoCO2;
  double tset_cold, tset_hot;

  /* conversion constants */
  par_frac = (double)0.45; /*fraction of insolation that is PAR - Mavi & Tupper
                              (2004)*/
  Wm2_umol = (double)4.56; /*conversion of solar radiation (W/m2) to umol/s/m2
                              photosynthetically active radiation*/
  second_per_day =
      (double)dayl *
      3600.; /* seconds per day length of day in hours and seconds per hour */
  ...
  Oi = (double)0.209 *
       parts_ppm;          /*oxygen at surface umol/mol to match Vmax, Jmax*/
  T = (double)met_list[1]; /*max temperature*/
  /*replaced: (double)0.5*(met_list[1]+met_list[0]);*/ /* mean temperature
                                                        */
  RH =
      (double)1. -
      met_list[4] /
          (6.1094 * exp((17.625 * T) / (T + 243.04))); /*relative humidity, <1*/

  tau = (double)2600. * pow(0.57, ((T - 25.) / 10.));
  gamma = (double)Oi /
          (2. *
           tau); /*co2 compensation point in ppm, umol/mol to match Vmax, Jmax*/

  Kc = (double)30. * pow(2.1, ((T - 25.) / 10.)) / psfc *
       parts_ppm; /*michaelis constant for carboxylation in ppm, umol/mol to
                     match Vmax, Jmax*/
  Ko = (double)30000. * pow(1.2, ((T - 25.) / 10.)) / psfc *
       parts_ppm; /*michaelis constant for oxidation in ppm, umol/mol to match
                     Vmax, Jmax*/

  /*compute temperature-adjusted carboxylation rate, apply temperature
   * limitations to Vmax25, Jmax25*/
  tset_cold =
      (double)10.; /*temperature for cold limitation [C], could be optimized*/
  tset_hot =
      (double)40.; /*temperature for hot limitation [C], could be optimized*/
  cold_inhibition = (double)1 + exp(0.25 * (tset_cold - T));
  heat_inhibition = (double)1 + exp(0.4 * (T - tset_hot));
  Vmax = (double)var_list[4] * pow(2.1, ((T - 25.) / 10.)) /
         (cold_inhibition * heat_inhibition);
  Jmax = (double)var_list[3] * pow(2.1, ((T - 25.) / 10.)) /
         (cold_inhibition * heat_inhibition);

  /*convert insolation (full shortwave from meteorology) to PAR absorbed by the
   * canopy*/
  PAR = (double)met_list[3] * 1e6 / second_per_day * Wm2_umol *
        par_frac; /* convert insolation from MJ m^-2 d^-1 to umol m^-2 s^-1 of
                     PAR, units for analytical setup*/

  /* Controlled by LAI and k, k = 0.6 picked from literature, could be optimized
   */
  APAR = (double)PAR *
         (1 - exp(-var_list[6] *
                  var_list[5])); /* calculate absorbed PAR as a exponential
                                    absorption in the canopy */
  ...
  Rd = (double)0.015 * Vmax * pow(2.4, ((T - 25.) / 10.)) /
       (1. + exp((1.3 * (T - 55.))));
  ...
  GPP_leaf_now = ANALYTICALSOLVER_BALDOCCHI(met_list, var_list, wc_adeb_list,
                                            wj_adeb_list, Rd, RH, ROOT_CALC);
  ...
  double met_list_high[] = {
      met_list[0], met_list[1], met_list[2] + 60., met_list[3],
      met_list[4]}; /* Switch in high value atmospheric CO2 */
  GPP_Ca_high = ANALYTICALSOLVER_BALDOCCHI(
      met_list_high, var_list, wc_adeb_list, wj_adeb_list, Rd, RH, ROOT_CALC);

  /* Response of GPP solution to CO2 */
  dGPPdCa = GPP_Ca_high - GPP_leaf_now;

  /***************************************/
  /* Screen for bad behavior of GPP, negative values or negative sensitivity to
   * CO2 slope*/
  ...
  /* as a function of m, GPP, RH, and CO2 at leaf + intercept, calc stomatal
   * conductance H20*/
  gs_H20 = (double)diffusionH2OtoCO2 * var_list[0] * GPP * RH / Cs +
           var_list[1]; /*gs is in mol m^-2 s^-1 of water*/
  ...
  lai_scaling = (double)(1 - exp(-var_list[6] * var_list[5])) /
                var_list[6]; /* unitless or m2/m2 */

  /* Scale leaf level GPP to canopy level using a big leaf exponential
   * approximation*/
  ...
  GPP_canopy_gC = (double)GPP_canopy * gram_per_mol_C * second_per_day /
                  1e6; /*1.03775, convert from umol m^-2 s^-1 to gC m^-2 d^-1*/
  ...
  /* Note: could apply the day length here in place of seconds per day*/
  Gs_canopy_kgH20 = (double)Gs_canopy_H20 * gram_per_mol_H20 * second_per_day /
                    gram_per_kg; // units of kg H2O m^-2 day^-1
  ...
  // iWUE=(double)GPP_scaled/(Gs_canopy*18.01528*86400./1000.); /*iWUE is in gC
  // / kgH2O

  static double
      return_arr[2]; /*need static variable to be able to return array?*/
