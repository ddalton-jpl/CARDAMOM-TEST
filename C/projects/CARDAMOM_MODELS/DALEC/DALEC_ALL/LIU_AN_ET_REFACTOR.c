...
int LIU_AN_ET(LIU_AN_ET_STRUCT *A) {
  ...
  // CONSTS
  double Ephoton =
      2.0e-25 / 500.0e-9; // Planck constant times speed of light (J.s*m.s-1)
                          // divided by light wavelength (m)
  ...
  double fT = pow(A->IN.q10canopyRd,
                  (T_C - 25.) / 10); // reference temperature is 25 degrees C

  Vcmax = vcmax25 * pow(q_10, 0.1 * (T_C - 25.)) /
          ((1 + exp(0.3 * (T_C - (Tupp - DGCM_TK0C)))) *
           (1 + exp(0.3 * ((Tdown - DGCM_TK0C) - T_C))));

  Jmax = Vcmax * exp(1.);

  J = (0.3 * PAR + Jmax -
       sqrt(pow(0.3 * PAR + Jmax, 2) - 4. * 0.9 * 0.3 * PAR * Jmax)) /
      2. / 0.9;
  ...
  A->OUT.LEAF_MORTALITY_FACTOR =
      fmax(1 - A->IN.NSC / (Rd_daily_potential * A->IN.deltat), 0);
  // Actual daily Rd
  ...
  // To scale from leaf to canopy, comment out the following line and uncomment
  // the one after double canopy_scale = 1.;

  // r[0] = An*canopy_scale*(12.e-6)*(24.*60.*60.); //from umolCO2m-2s-1 to
  // gCm-2day-1
  ...
  double SRADg;
  double sV; // sV is the rate of change of saturated vapor pressure with air
             // temperature in degC.
  double petVnum;
  double petVnumB;
  ...
  SRADg = (1. - 0.5 * (leaf_refl_par + leaf_refl_nir)) * SRAD *
          exp(-VegK * LAI * clumping);

  SRAD = (1. - 0.5 * (leaf_refl_par + leaf_refl_nir)) * SRAD;

  petVnum =
      (sV * (SRAD - SRADg) + 1.225 * 1000 * VPD_kPa * ga) / lambda0 * 60 * 60;
  petVnumB =
      1.26 * (sV * SRADg) / (sV + gammaV) / lambda0 * 60 * 60; // from mm.hr-1
