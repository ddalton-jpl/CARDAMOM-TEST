...
typedef struct {
  struct {
    ...
    double t_c;
    double t_r;
    double T_memory; // udpated each iteration (automatically), can also be
                     // externally prescribed, no functional difference.
    double lambda_max_memory; // updated each iteration (automatically), can
                              // also be externally prescribed, no functional
                              // difference.
  } IN;
  struct {
    double lambda_next;
...
int KNORR_ALLOCATION(KNORR_ALLOCATION_STRUCT *K) {
  ...
  tau_W = K->IN.tau_W;
  lambda_max_memory =
      K->IN.lambda_max_memory; // TIME-DEPENDENT I.E. SAVE IN MEMORY
  tau_s = 30.0; // Averaging period for water/structural memory (units of days;
                // must be in same units as deltat), usually kept constant but
                // could potentially be a tuneable parameter
  t_c = K->IN.t_c;
  t_r = K->IN.t_r;
  ...
  lambda_W =
      (pasm * lambda) / (tau_W * MaxExponentialSmooth(transp, 1e-3, 2e-2));
