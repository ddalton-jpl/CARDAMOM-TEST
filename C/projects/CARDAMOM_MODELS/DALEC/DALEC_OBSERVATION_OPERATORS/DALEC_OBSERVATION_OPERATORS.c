...
typedef struct OBSOPE {
  ...
  int GPP_flux;
  // shuang added SIF operator so that both GPP and SIF can be compared at the
  // same time, with different observation datasets, eg. TROPOMI SIF and FluxSat
  // GPP
...
int DALEC_OBSOPE_ABGB(DATA *D, OBSOPE *O) {
  ...
  if (TOBS.valid_obs_length > 0 || SOBS.value != DEFAULT_DOUBLE_VAL) {
    ...
    for (n = 0; n < N; n++) {
      ...
      for (nn = 0; nn < O->ABGB_n_pools; nn++) {
        D->M_ABGB[n] += (D->M_POOLS[p + O->ABGB_pools[nn]] +
                         D->M_POOLS[nxp + O->ABGB_pools[nn]]) *
                        0.5;
      }
    }
  }
  ...
  if (SOBS.validobs) {
    ...
    D->M_Mean_ABGB = 0;
    for (n = 0; n < N; n++) {
      D->M_Mean_ABGB += D->M_ABGB[n];
    };
    D->M_Mean_ABGB = D->M_Mean_ABGB / (double)N;
  };
...
int DALEC_OBSOPE_CUEmrg(DATA *D, OBSOPE *O) {
  ...
  double MGPP =
      0; // Initializing as zero, to allow for loop averaging calculation
  double MRauto =
      0; // Initializing as zero, to allow for loop averaging calculation
  ...
  if (TOBS.validobs) {
    ...
    for (n = 0; n < N; n++) {
      ...
      for (nn = 0; nn < O->EWT_n_h2o_pools; nn++) {
        D->M_EWT[n] += (D->M_POOLS[D->nopools * n + h2op[nn]] +
                        D->M_POOLS[D->nopools * (n + 1) + h2op[nn]]) *
                       0.5;
      }
    }
  }
...
// SIF operator is made the same as GPP, be careful with its opt_filter and
// opt_normalization set up - shuang Mean_SIF is not available since current
// model does not yet simulate SIF - shuang

int DALEC_OBSOPE_SIF(DATA *D, OBSOPE *O) {

  // SIF timeseries length, here DATA refer to input obs, OBSOPE refer to model
  // output mapped to mathch data
  int N = D->ncdf_data.TIME_INDEX.length;
  ...
  if (TOBS.validobs) {
    ...
    for (n = 0; n < N; n++) {
      ...
      for (nn = 0; nn < O->NBE_n_fluxes; nn++) {
        D->M_NBE[n] += D->M_FLUXES[D->nofluxes * n + O->NBE_fluxes[nn]] *
                       O->NBE_flux_signs[nn];
      }
    }
  };
...
int DALEC_OBSOPE_LAI(DATA *D, OBSOPE *O) {
  ...
  if ((TOBS.valid_obs_length > 0 || SOBS.value != DEFAULT_DOUBLE_VAL)) {
    int n;
    for (n = 0; n < N; n++) {
      D->M_LAI[n] = (D->M_POOLS[D->nopools * n + O->LAI_pool] +
                     D->M_POOLS[D->nopools * (n + 1) + O->LAI_pool]) *
                    0.5;
    }
  }
...
int DALEC_OBSOPE_SCF(DATA *D, OBSOPE *O) {
  ...
  if ((TOBS.valid_obs_length > 0)) {
    ...
    for (n = 0; n < N; n++) {
      ...
      D->M_SCF[n] = (D->M_POOLS[D->nopools * n + O->SCF_pool] +
                     D->M_POOLS[D->nopools * (n + 1) + O->SCF_pool]) *
                    0.5;
    }
  }
  ...
  if (TOBS.valid_obs_length > 0) {
    ...
    for (n = 0; n < N; n++) {
      ...
      for (nn = 0; nn < O->CWOO_n_pools; nn++) {
        D->M_CWOO[n] += (D->M_CWOO[p + O->CWOO_pools[nn]] +
                         D->M_CWOO[nxp + O->CWOO_pools[nn]]) *
                        0.5;
      }
    }
  }
  ...
  if (TOBS.valid_obs_length > 0) {
    ...
    for (n = 0; n < N; n++) {
      ...
      for (nn = 0; nn < O->DOM_n_pools; nn++) {
        D->M_DOM[n] += (D->M_POOLS[p + O->DOM_pools[nn]] +
                        D->M_POOLS[nxp + O->DOM_pools[nn]]) *
                       0.5;
      }
    }
  }
...
int DALEC_OBSOPE(DATA *D, OBSOPE *O) {
  ...
  if (O->SUPPORT_ABGB_OBS) {
    DALEC_OBSOPE_ABGB(D, O);
  }
  if (O->SUPPORT_CH4_OBS) {
    DALEC_OBSOPE_CH4(D, O);
  }
  if (O->SUPPORT_CWOO_OBS) {
    DALEC_OBSOPE_CWOO(D, O);
  }
  if (O->SUPPORT_ET_OBS) {
    DALEC_OBSOPE_ET(D, O);
  }
  if (O->SUPPORT_EWT_OBS) {
    DALEC_OBSOPE_EWT(D, O);
  }
  if (O->SUPPORT_FIR_OBS) {
    DALEC_OBSOPE_FIR(D, O);
  }
  if (O->SUPPORT_GPP_OBS) {
    DALEC_OBSOPE_GPP(D, O);
  }
  if (O->SUPPORT_LAI_OBS) {
    DALEC_OBSOPE_LAI(D, O);
  }
  if (O->SUPPORT_NBE_OBS) {
    DALEC_OBSOPE_NBE(D, O);
  }
  if (O->SUPPORT_ROFF_OBS) {
    DALEC_OBSOPE_ROFF(D, O);
  }
  if (O->SUPPORT_SCF_OBS) {
    DALEC_OBSOPE_SCF(D, O);
  }

  // Emergent quantities
  if (O->SUPPORT_CUEmrg_OBS) {
    DALEC_OBSOPE_CUEmrg(D, O);
  }

  // Parameters

  if (O->SUPPORT_Cefficiency_OBS) {
    DALEC_OBSOPE_Cefficiency(D, O);
  }
  if (O->SUPPORT_CUE_OBS) {
    DALEC_OBSOPE_CUE(D, O);
  }
  if (O->SUPPORT_C3frac_OBS) {
    DALEC_OBSOPE_C3frac(D, O);
  }
  if (O->SUPPORT_Vcmax25_OBS) {
    DALEC_OBSOPE_Vcmax25(D, O);
  }
  if (O->SUPPORT_iniSnow_OBS) {
    DALEC_OBSOPE_iniSnow(D, O);
  }
  if (O->SUPPORT_iniSOM_OBS) {
    DALEC_OBSOPE_iniSOM(D, O);
  }
  if (O->SUPPORT_LCMA_OBS) {
    DALEC_OBSOPE_LCMA(D, O);
  }
