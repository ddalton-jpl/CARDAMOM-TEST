...
int DALEC_101(DATA DATA, double const *pars) {
  ...
  double constants[10] = {pars[4], 0.0156935, 4.22273,  208.868, 0.0453194,
  ...
  */
  ...
  for (n = 0; n < nr; n++) {
    ...
    FLUXES[f + 6] =
        FLUXES[f + 0] - FLUXES[f + 2] - FLUXES[f + 5] - FLUXES[f + 4];
    /*leaf fall factor*/
    FLUXES[f + 8] =
        (2 / sqrt(pi)) * (ff / wf) *
        exp(-pow(sin((DATA.MET[m + 0] - pars[8] + osf) / sf) * sf / wf, 2));
    /*Labrelease factor*/
    FLUXES[f + 15] =
        (2 / sqrt(pi)) * (fl / wl) *
        exp(-pow(sin((DATA.MET[m + 0] - pars[5] + osl) / sf) * sf / wl, 2));
    ...
    FLUXES[f + 12] =
        0; /*POOLS[p+4]*(1-pow(1-FLUXES[f+1]*pars[8-1],deltat))/deltat*/
    ;
    /*respiration heterotrophic SOM*/
    FLUXES[f + 13] =
        0; /*POOLS[p+5]*(1-pow(1-FLUXES[f+1]*pars[9-1],deltat))/deltat*/
    ;
    FLUXES[f + 13] =
        POOLS[p + 2] * (1 - pow(1 - FLUXES[f + 1] * pars[0], deltat)) / deltat;
    /*litter to SOM*/
    FLUXES[f + 14] =
        0; /*POOLS[p+4]*(1-pow(1-pars[1-1]*FLUXES[f+1],deltat))/deltat;*/
    ...
    POOLS[nxp + 2] = POOLS[p + 2] +
                     (FLUXES[f + 6] + FLUXES[f + 9] - FLUXES[f + 13]) * deltat;
    /*POOLS[nxp+3] = POOLS[p+3] +  (FLUXES[f+6] - FLUXES[f+10])*deltat;
    POOLS[nxp+4] = POOLS[p+4] + (FLUXES[f+9] + FLUXES[f+11] - FLUXES[f+12] -
    FLUXES[f+14])*deltat; POOLS[nxp+5]= POOLS[p+5]+ (FLUXES[f+14] -
