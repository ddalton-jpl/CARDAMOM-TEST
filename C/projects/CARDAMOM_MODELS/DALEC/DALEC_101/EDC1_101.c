...
/*EDC1 contains all checks that do not require a full DALEC_CDEA (DALEC2) model
 * run*/
 ...
 * EDCD->SWITCH[1-1]==1)) & (pars[8]>pars[7])){EDC=0;EDCD->PASSFAIL[1-1]=0;}
 */
/*EDC CHECK NO 2*/
/*Litter2SOM greater than SOM to Atm. rate*/
/*if (((EDC==1 & DIAG==0) || DIAG==1 || (EDC==1 & DIAG==2 &
 * EDCD->SWITCH[2-1]==1)) & (pars[0]<pars[8])){EDC=0;EDCD->PASSFAIL[2-1]=0;}
 */
/*EDC CHECK NO 3*/
/*TOR_FOL faster than TOR_WOOD */
/*if (((EDC==1 & DIAG==0) || DIAG==1 || (EDC==1 & DIAG==2 &
 * EDCD->SWITCH[3-1]==1)) & (pars[5]>torfol)){EDC=0;EDCD->PASSFAIL[3-1]=0;}
 */
/*EDC CHECK NO 4*/
...
/*EDC CHECK NO 5*/
/*if (((EDC==1 & DIAG==0) || DIAG==1 || (EDC==1 & DIAG==2 &
 * EDCD->SWITCH[5-1]==1)) & ((ffol+flab)>5*froot |
 * (ffol+flab)*5<froot)){EDC=0;EDCD->PASSFAIL[5-1]=0;}
 */
/*EDC No 8*/
