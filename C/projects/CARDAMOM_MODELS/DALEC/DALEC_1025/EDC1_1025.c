...
int EDC1_1025(double const *pars, DATA DATA, struct EDCDIAGNOSTIC *EDCD) {
  ...
  */
  ...
  // double const ffol=(1-fauto)*pars[P.f_foliar];  // Removed this as there is
  // no longer a direct flux from NPP -> Cfol, it all goes via Clab
  ...
  double const fsom = fwood + (froot + flab) * pars[P.tr_lit2soil] /
                                  (pars[P.tr_lit2soil] + pars[P.t_lit]);
  ...
  if (((EDC == 1 & DIAG == 0) || DIAG == 1 ||
       (EDC == 1 & DIAG == 2 & EDCD->SWITCH[1 - 1] == 1)) &
      (pars[P.t_soil] > pars[P.t_lit])) {
    EDC = 0;
    EDCD->PASSFAIL[1 - 1] = 0;
  }
  ...
  if (((EDC == 1 & DIAG == 0) || DIAG == 1 ||
       (EDC == 1 & DIAG == 2 & EDCD->SWITCH[2 - 1] == 1)) &
      (pars[P.tr_lit2soil] < pars[P.t_soil])) {
    EDC = 0;
    EDCD->PASSFAIL[2 - 1] = 0;
  }
  ...
  // EDCD->SWITCH[3-1]==1)) &
  // (pars[P.t_wood]>torfol)){EDC=0;EDCD->PASSFAIL[3-1]=0;}
  ...
  // if (((EDC==1 & DIAG==0) || DIAG==1 || (EDC==1 & DIAG==2 &
  // EDCD->SWITCH[4-1]==1)) &
  // (pars[P.t_root]<pars[P.t_soil]*exp(pars[P.temp_factor]*meantemp))){EDC=0;EDCD->PASSFAIL[4-1]=0;}

  /*EDC no 5 is addressed in EDC2_FIREBUCKET.c*/

  /*EDC CHECK NO 6*/
  /*Bday Fday difference>45 days */
  /*if (((EDC==1 & DIAG==0) || DIAG==1) & (fabs(pars[P.Fday] - pars[P.Bday])<45
   * | fabs(pars[P.Fday]-pars[P.Bday])>320.25)){EDC=0;EDCD->PASSFAIL[6-1]=0;}*/
  /*now obsolete!*/

  /*EDC CHECK NO 5*/
  /*Allocation to canopy is comparable to allocation to fine roots*/
  // if (((EDC==1 & DIAG==0) || DIAG==1 || (EDC==1 & DIAG==2 &
  // EDCD->SWITCH[5-1]==1)) & ((flab)>5*froot |
  // (flab)*5<froot)){EDC=0;EDCD->PASSFAIL[5-1]=0;}

  /*EDC No 16* Foliage CF> wood CF & SOM CF*/
  if (((EDC == 1 & DIAG == 0) || DIAG == 1 ||
       (EDC == 1 & DIAG == 2 & EDCD->SWITCH[16 - 1] == 1)) &
      (pars[P.cf_foliar] < pars[P.cf_ligneous] |
       pars[P.cf_foliar] < pars[P.cf_DOM])) {
    EDC = 0;
    EDCD->PASSFAIL[16 - 1] = 0;
  }

  /*EDC No 17: RD_puw > RD_paw*/
  // if (((EDC==1 & DIAG==0) || DIAG==1 || (EDC==1 & DIAG==2 &
  // EDCD->SWITCH[17-1]==1)) &
  // (pars[P.PAW_Qmax]<pars[P.P])){EDC=0;EDCD->PASSFAIL[17-1]=0;}
