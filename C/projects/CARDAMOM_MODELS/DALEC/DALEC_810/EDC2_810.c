...
int EDC2_810(double const *pars, DATA DATA, struct EDCDIAGNOSTIC *EDCD) {
  ...
  */
   ...
   * MPOOLS[1]*5<MPOOLS[2])){EDC=ipow(0,EDCD->SWITCH[6-1]);EDCD->PASSFAIL[6-1]=0;
   * }
   */
  ...
  /*if (((EDC==1 & DIAG==0) || DIAG==1 || (EDC==1 & DIAG==2 &
   * EDCD->SWITCH[7-1+n]==1)) &
   * (pars[25]>MPOOLS[6])){EDC=ipow(0,EDCD->SWITCH[14-1]);EDCD->PASSFAIL[14-1]=0;}
   */
  /***********************EDCs done here****************************/
  EDCD->pEDC =
      EDCD->pEDC + log(1 / (1 + exp(10 * (pars[25] - MPOOLS[6]) / MPOOLS[6])));
