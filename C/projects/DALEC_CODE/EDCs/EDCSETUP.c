...
int EDCSETUP(DATA DATA, struct EDCDIAGNOSTIC **EDCD) {
  ...
  printf("*****")
  printf("*****")
  printf("******Only default values used for now, introduce EDC settings via
  netcdf system when needed\n"); printf("*****")
  */

  for (n = 0; n < 100; n++) {
    EDCDmem.SWITCH[n] = 1;
  }
  // if (EDCDmem.DIAG==2){for
  // (n=0;n<20;n++){EDCDmem.SWITCH[n]=DATA.otherpriors[n+30];}}
  ...
  EDCDmem.nedc = 100;
  for (n = 0; n < EDCDmem.nedc; n++) {
    EDCDmem.PASSFAIL[n] = 1;
  }
