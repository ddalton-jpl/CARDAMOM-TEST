...
// Helper method, primarily for use identifying file extensions. Adapted from
// https://stackoverflow.com/questions/744766/how-to-compare-ends-of-strings-in-c
// for ease and edge case management
...
int DYNAMIC_DATA_MEMORY_ALLOCATION(DATA *DATA) {
  ...
  DATA->M_LAI =
      calloc(Ntimesteps, sizeof(double)); // Warning, LAI larger than this!
...
int CARDAMOM_READ_BINARY_DATA(char *ncfilename, DATA *DATA) {
  ...
  //
  // printf("DATA->ncdf_data.ET.opt_unc_type =
  // %i\n",DATA->ncdf_data.ET.opt_unc_type);
  // printf("DATA->ncdf_data.ET.opt_filter =
  // %i\n",DATA->ncdf_data.ET.opt_filter); printf("DATA->ncdf_data.ET.single_unc
  // = %2.2f\n",DATA->ncdf_data.ET.single_unc);
  // printf("DATA->ncdf_data.ET.min_threshold = %2.2f\n",
  // DATA->ncdf_data.ET.min_threshold); printf("Successfully pre-processed
  // ET...\n");
