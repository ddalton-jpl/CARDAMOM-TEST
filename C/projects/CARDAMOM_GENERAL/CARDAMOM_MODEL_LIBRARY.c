...
/*General note: The code IDxMA (where x is the model ID) is kept above the model
attribute line. The reason for this is to ensure that Matlab can easily access
this code*/
...
int CARDAMOM_MODEL_LIBRARY(DATA *DATA) {
  ...
  if (ID == 0) {
    printf("No model prescribed, expect error!");
  }
