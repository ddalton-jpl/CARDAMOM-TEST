...
// This is a tiny macro used to do error handeling for netCDF methods with the
// standard format of returning a result code. It helps make the code look less
// insane by allowing one-line calls This as a macro because we want the
// __FILE__ and __LINE__ macros to work
#define FAILONERROR(ncretval)                                                  \
  if (ncretval != NC_NOERR) {                                                  \
    NCDFERR(ncretval);                                                         \
    exit(ERREXITCODE);                                                         \
  }
// This variant will not die, but will still thow a message at the user about
// the problem.
#define WARNONERROR(ncretval)                                                  \
  if (ncretval != NC_NOERR) {                                                  \
    NCDFERR(ncretval);                                                         \
  }

// This scans the string and removes all instances of the string toFind, and
// replaces them with the single char toReplace.
void str_inplace_replace(char *str, const char *toFind, const char toReplace) {
  // Yeah this implementation is N^2... but we have a small fixed max N, so
  // don't @ me.
  char *substring = str;
  while ((substring = strstr(substring, toFind))) {
    memmove(substring, substring + 1,
            strlen(substring)); // Memmove needed due to overlapping strings.
                                // Move the whole string, with null term
...
/*NOTE: this function can be used with any valid model ID (stored in
 * CARDADATA.ID)*/
int main(int argc, char *files[]) {
  ...
  strncpy(metfile, files[1], FILE_NAME_MAX_LEN - 1);
  char parfile[FILE_NAME_MAX_LEN];
  strncpy(parfile, files[2], FILE_NAME_MAX_LEN - 1);
  char ncdffile[FILE_NAME_MAX_LEN];
  if (argc - 1 > 2) {
    strncpy(ncdffile, files[3], FILE_NAME_MAX_LEN - 1);
  } else {
    int ncFilenameLen = strlen(parfile) - 4;
    if (ncFilenameLen > FILE_NAME_MAX_LEN - 8) {
      printf("Error on input validation, parfile name too long to create "
             "netCDF file name from.\n");
      exit(1);
    }
    strncpy(ncdffile, parfile, ncFilenameLen);
    strncpy(ncdffile + ncFilenameLen, ".cbr.nc",
            ncFilenameLen + 1); // Write an extra char so strncpy adds a null
  ...
  /*NOTE: parameter minimum and maximum values also loaded as a function of
  CARDADATA.ID by the CARDAMOM_MODEL_LIBRARY.c function*/
  ...
  int verbal = 1;
  if (verbal == 1) {
    printf("number of parvecs to run = %d\n", N);
  }

  /*resetting file reader to start*/
  fseek(fd, 0, SEEK_SET);
  /*parameter file will be closed later - in the meantime parameters will simply
   * be read N times*/
  ...
  int ncretval =
      0; // This is a reused variable for the return value of ncdf methods.
  ...
  int sampleDimID, poolDimID, fluxDimID, timePoolsDimID, timeFluxesDimID,
      probIdxDimID, edcIdxDimID, noParsDimID, noLikelihoodsDimID;
  ...
  // NOTE: this was going to be the NC_UNLIMITED dimension, however due to
  // concerns with support for netcdf classic, it is now fixed, and split into
  // two
  ...
  FAILONERROR(nc_def_dim(ncid, "Likelihood Index", CARDADATA.nolikelihoods,
                         &noLikelihoodsDimID));
  ...
  WARNONERROR(nc_put_att_text(ncid, fluxesVarID, "example_lowercase_name",
                              strlen(fluxesLowercaseName),
                              fluxesLowercaseName));
  // This is an example of an array of doubles. Yes, you do need to specify
  // NC_DOUBLE even though we used the type-safe method nc_put_att_double
  WARNONERROR(nc_put_att_double(ncid, fluxesVarID, "example_doubles", NC_DOUBLE,
                                4, (double[]){12.44, 441.0, 3.14159265, 0.0}));
  ...
  FAILONERROR(nc_def_var(ncid, "LIKELIHOODS", NC_DOUBLE, 2, likelihoods_dems,
                         &likelihoodsVarID));
  ...
  for (n = 0; n < N; n++) {
    ...
    /*step 4.2 - running MODEL_LIKELIHOOD_FUNCTION (e.g. DALEC_CDEA_MLF) with
     * parameter vector from file*/
    ...
    // printf("DATA.LAI.values[24] =
    // %2.2f\n",CARDADATA.ncdf_data.LAI.values[24]); printf("DATA.M_POOLS[1] =
    // %2.2f\n",CARDADATA.M_POOLS[1]);
    ...
    // //**********Variable attributes*************
    // int opt_unc_type;//(0 = absolute sigma, 1 = uncertainty factor, 2 = sigma
    // as fraction of value) int opt_normalization;//(0 = none, 1 = remove mean,
    // 2 = divide by mean) int opt_filter;//(0 = no filter, 1 = mean only,
    // 2==annual mean & monthly anomaly, 3 = seasonal cycle & inter-annual
    // anomalies). double min_threshold;//Minimum value threshold: model and/or
    // data will be rounded up to this value (default = -inf) double
    // single_monthly_unc;//Fields to be used only with Filter=2 AND
    // opt_unc_type=0; double single_annual_unc;//Fields to be used only with
    // Filter=2 AND opt_unc_type=0; double single_mean_unc;//Fields to be used
    // only with Filter = 1; double single_unc;// double structural_unc;//this
    // gets added to uncertainty in quadrature.
    ...
    // Write fluxes
    FAILONERROR(
        nc_put_vara_double(ncid, fluxesVarID, (const size_t[]){n, 0, 0},
                           (const size_t[]){1, Ntimesteps, CARDADATA.nofluxes},
                           CARDADATA.M_FLUXES));
    // Write pools
    FAILONERROR(nc_put_vara_double(
        ncid, poolsVarID, (const size_t[]){n, 0, 0},
        (const size_t[]){1, Ntimesteps + 1, CARDADATA.nopools},
        CARDADATA.M_POOLS));
    // write edcd

    FAILONERROR(nc_put_vara_double(ncid, edcsVarID, (const size_t[]){n, 0},
                                   (const size_t[]){1, CARDADATA.noedcs},
                                   CARDADATA.M_EDCs));
    // write M_P
    FAILONERROR(nc_put_vara_double(ncid, pVarID, (const size_t[]){n, 0},
                                   (const size_t[]){1, probIdxLen},
                                   CARDADATA.M_P));
    // write Pars
    FAILONERROR(nc_put_vara_double(ncid, parsVarID, (const size_t[]){n, 0},
                                   (const size_t[]){1, CARDADATA.nopars},
                                   pars));
    // Write Likelihoods
    FAILONERROR(nc_put_vara_double(
        ncid, likelihoodsVarID, (const size_t[]){n, 0},
        (const size_t[]){1, CARDADATA.nolikelihoods}, CARDADATA.M_LIKELIHOODS));

    // for (n=0;n<CARDADATA.nolikelihoods;n++){printf("%5.4f
    // ",CARDADATA.M_LIKELIHOODS[n]);} printf("\n");
