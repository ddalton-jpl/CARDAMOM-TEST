...
// Note that a lot of this is derived from
// https://www.unidata.ucar.edu/software/netcdf/docs/simple_nc4_rd_8c-example.html
 ...
 *
 *
 *	NOTE: if you intend to modify what is read in when reading a netCDF
 *file, You MUST edit both this method, and the DATA struct in
...
int CARDAMOM_READ_NETCDF_DATA(char *filename, NETCDF_DATA *DATA) {
  ...
  // default_int_value(OBS.opt_normalization,0);//(0 = none, 1 = remove mean, 2
  // = divide by mean) default_int_value(OBS.opt_filter,0);
  ...
  DATA->PEQ_Cefficiency =
      READ_NETCDF_SINGLE_OBS_FIELDS(ncid, "PEQ_Cefficiency");
  ...
  printf("Warning: No longer assigning default uncertainty values. Please "
         "double check your driver file contains uncertainties for all "
         "observations used.\n");
  printf("Example uncertainty values (used in the past) per variable:\n");
  // Default ABGB options
  printf(
      "ABGB.opt_unc_type=1; ABGB.single_unc=2; ABGB.min_threshold=10 gC/m2;\n");
  ...
  // Default CH4 options
  printf("CH4.opt_unc_type=1; CH4.single_unc=2; CH4.min_threshold=1e-5 "
         "mgCH4/m2/d;\n");
  ...
  printf(
      "ROFF.opt_unc_type=1; ROFF.single_unc=2; ROFF.min_threshold=0.1 mm/d;\n");
  ...
  // Default GPP options
  printf(
      "GPP.opt_unc_type=1; GPP.single_unc=2; GPP.min_threshold=0.1 gC/m2/d;\n");
  ...
  printf(
      "SIF.opt_unc_type=1; SIF.single_unc=2; SIF.min_threshold=0.1 gC/m2/d;\n");
  ...
  // Default LAI options
  printf(
      "LAI.opt_unc_type=1; LAI.single_unc=2; LAI.min_threshold=0.1 m2/m2;\n");
  ...
  // Default SCF options
  printf(
      "SCF.opt_unc_type=0; SCF.single_unc=0.1; SCF.min_threshold=0.1 m2/m2;\n");
  ...
  DATA->BURNED_AREA.values =
      ncdf_read_double_var(ncid, "BURNED_AREA", &(DATA->BURNED_AREA.length));
  DATA->BURNED_AREA.reference_mean =
      ncdf_read_double_attr(ncid, "BURNED_AREA", "reference_mean");
  DEFAULT_REFERENCE_MEAN(&DATA->BURNED_AREA);

  DATA->CO2.values = ncdf_read_double_var(ncid, "CO2", &(DATA->CO2.length));
  DATA->CO2.reference_mean =
      ncdf_read_double_attr(ncid, "CO2", "reference_mean");
  DEFAULT_REFERENCE_MEAN(&DATA->CO2);

  DATA->DISTURBANCE_FLUX.values = ncdf_read_double_var(
      ncid, "DISTURBANCE_FLUX", &(DATA->DISTURBANCE_FLUX.length));
  DATA->DISTURBANCE_FLUX.reference_mean =
      ncdf_read_double_attr(ncid, "DISTURBANCE_FLUX", "reference_mean");
  DEFAULT_REFERENCE_MEAN(&DATA->DISTURBANCE_FLUX);

  DATA->DOY.values = ncdf_read_double_var(ncid, "DOY", &(DATA->DOY.length));
  // DATA->DOY.reference_mean=ncdf_read_double_attr(ncid,
  // "DOY","reference_mean");

  DATA->SKT.values = ncdf_read_double_var(ncid, "SKT", &(DATA->SKT.length));
  DATA->SKT.reference_mean =
      ncdf_read_double_attr(ncid, "SKT", "reference_mean");
  DEFAULT_REFERENCE_MEAN(&DATA->SKT);

  DATA->SNOWFALL.values =
      ncdf_read_double_var(ncid, "SNOWFALL", &(DATA->SNOWFALL.length));
  DATA->SNOWFALL.reference_mean =
      ncdf_read_double_attr(ncid, "SNOWFALL", "reference_mean");
  DEFAULT_REFERENCE_MEAN(&DATA->SNOWFALL);

  DATA->SSRD.values = ncdf_read_double_var(ncid, "SSRD", &(DATA->SSRD.length));
  DATA->SSRD.reference_mean =
      ncdf_read_double_attr(ncid, "SSRD", "reference_mean");
  DEFAULT_REFERENCE_MEAN(&DATA->SSRD);

  DATA->STRD.values = ncdf_read_double_var(ncid, "STRD", &(DATA->STRD.length));
  DATA->STRD.reference_mean =
      ncdf_read_double_attr(ncid, "STRD", "reference_mean");
  DEFAULT_REFERENCE_MEAN(&DATA->STRD);

  DATA->T2M_MAX.values =
      ncdf_read_double_var(ncid, "T2M_MAX", &(DATA->T2M_MAX.length));
  DATA->T2M_MAX.reference_mean =
      ncdf_read_double_attr(ncid, "T2M_MAX", "reference_mean");
  DEFAULT_REFERENCE_MEAN(&DATA->T2M_MAX);

  DATA->T2M_MIN.values =
      ncdf_read_double_var(ncid, "T2M_MIN", &(DATA->T2M_MIN.length));
  DATA->T2M_MIN.reference_mean =
      ncdf_read_double_attr(ncid, "T2M_MIN", "reference_mean");
  DEFAULT_REFERENCE_MEAN(&DATA->T2M_MIN);

  DATA->TIME_INDEX.values =
      ncdf_read_double_var(ncid, "time", &(DATA->TIME_INDEX.length));
  // DATA->TIME_INDEX.reference_mean=ncdf_read_double_attr(ncid,
  // "time","reference_mean");

  DATA->TOTAL_PREC.values =
      ncdf_read_double_var(ncid, "TOTAL_PREC", &(DATA->TOTAL_PREC.length));
  DATA->TOTAL_PREC.reference_mean =
      ncdf_read_double_attr(ncid, "TOTAL_PREC", "reference_mean");
  DEFAULT_REFERENCE_MEAN(&DATA->TOTAL_PREC);

  DATA->VPD.values = ncdf_read_double_var(ncid, "VPD", &(DATA->VPD.length));
  DATA->VPD.reference_mean =
      ncdf_read_double_attr(ncid, "VPD", "reference_mean");
  DEFAULT_REFERENCE_MEAN(&DATA->VPD);

  DATA->YIELD.values =
      ncdf_read_double_var(ncid, "YIELD", &(DATA->YIELD.length));
  DATA->YIELD.reference_mean =
      ncdf_read_double_attr(ncid, "YIELD", "reference_mean");
  ...
  // Mean temp
  DATA->meantemp =
      DATA->T2M_MIN.reference_mean * 0.5 + DATA->T2M_MAX.reference_mean * 0.5;
  // Solar Zenith Angle
  ...
  for (n = 0; n < DATA->Ntimesteps; n++) {
    ...
    double DA = 23.45 * sin((284 + DATA->DOY.values[n]) * 2 * pi /
                            365); // Deviation angle
    // double LST = (int) (DOY[n]*24*60) % (24*60);
    double LST = 0.5 * 24 * 60;
    double AST = LST + ET1;
    double h = (AST - 12 * 60) / 4; // hour angle
    double alpha = asin((sin(pi / 180 * DATA->LAT) * sin(pi / 180 * DA) +
                         cos(pi / 180 * DATA->LAT) * cos(pi / 180. * DA) *
                             cos(pi / 180 * h))) *
  ...
  MCMCID.nSAMPLES_EDC_SEARCH =
      ncdf_read_int_attr(ncid, "MCMCID", "nSAMPLES_EDC_SEARCH");
  ...
  if (isnan(MCMCID.value)) {
    MCMCID.value = DEFAULT_DOUBLE_VAL;
  }
