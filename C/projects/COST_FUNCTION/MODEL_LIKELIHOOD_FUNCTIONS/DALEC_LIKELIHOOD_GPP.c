  ...
  if (D.ngpp > 0) {
    ...
    if (D.gpp_annual_unc < 1) {
      ...
      for (n = 0; n < D.ngpp; n++) {
        dn = D.gpppts[n];
        tot_exp +=
            pow(log(max(D.M_GPP[dn] / mgpp, th) / max(D.GPP[dn] / msif, th)) /
                    log(D.gpp_obs_unc),
                2);
      }
    } else {
      ...
      for (m = 0; m < D.ngpp / 12; m++) {
        ...
        for (n = 0; n < 12; n++) {
          dn = D.gpppts[n + m * 12];
          mam = mam + D.M_GPP[dn] / mgpp;
          am = am + D.GPP[dn] / msif;
        }
        /*Calculate seasonal cost function*/
        for (n = 0; n < 12; n++) {
          dn = D.gpppts[n + m * 12];
          tot_exp += pow(log(max(D.M_GPP[dn] / mgpp, th) / mam /
                             (max(D.GPP[dn] / msif, th) / am)) /
                             log(D.gpp_obs_unc),
                         2);
        }
