double DALEC_LIKELIHOOD_LAI(DATA D) {
  ...
  int n, dn;
  /*LAI likelihood*/
  /*For future reference: average of model LAI at t and t+1 likely more
   * appropriate*/
  /*
  ...
  */
  ...
  // Notes: If no "MLAI" (mean LAI) is provided, then cost function conducts a
  ...
  // NOTES: if "MLAI" (mean LAI) is provided, then only mean LAI is considered
  // (either throughout model run or throughout overlapping timeseries,
  // depending on whether LAI timeseries data is provided.
  else if (D.otherpriors[4] > 0) {

    // If timeseries data is provided, calculate model MLAI constraint during
    // those points
    if (D.nlai > 0) {
      for (n = 0; n < D.nlai; n++) {
        dn = D.laipts[n];
    ...
    // Apply MLAI as normal or log-normal distrubution (depending if MLAI unc is
