...
int EDCSETUP(DATA DATA, struct EDCDIAGNOSTIC **EDCD) {
  ...
  /*switch all on if EDCDIAG<2 , otherwise, switch on/off according to
   * EDCSWITCH*/
  ...
  if (EDCDmem.DIAG == 2) {
    for (n = 0; n < 20; n++) {
      EDCDmem.SWITCH[n] = DATA.otherpriors[n + 30];
    }
  }
...
double LIKELIHOOD_P(DATA DATA, double *PARS) {
  ...
  for (n = 0; n < 50; n++) {
    if (DATA.parpriors[n] > -9999 & DATA.parpriorunc[n] > -9999) {
      p = p -
          0.5 * pow(log(PARS[n] / DATA.parpriors[n]) / log(DATA.parpriorunc[n]),
                    2);
    }
  }
  ...
  if (DATA.otherpriors[0] > -9999) {
    p = p - 0.5 * pow(log((PARS[17] + PARS[18] + PARS[19] + PARS[20]) /
                          DATA.otherpriors[0]) /
                          log(DATA.otherpriorunc[0]),
                      2);
  }
...
double LIKELIHOOD(DATA D) {
  ...
  if (D.ngpp > 0) {
    ...
    if (D.gppabs == 1) {
      ...
      for (n = 0; n < D.ngpp; n++) {
        dn = D.gpppts[n];
        tot_exp += pow((D.M_GPP[dn] - D.GPP[dn]) / 2, 2);
      }
      P = P - 0.5 * tot_exp;
    } else if (D.gppabs == 0) {
      /*GPP relative*/
      ...
      for (n = 0; n < D.ngpp; n++) {
        dn = D.gpppts[n];
        tot_exp +=
            pow(log(D.M_GPP[dn] / mgpp / (D.GPP[dn] / msif)) / log(2), 2);
      }
      P = P - 0.5 * tot_exp;
    }
  }
  ...
  /*LAI likelyhood*/
  tot_exp = 0;
  if (D.nlai > 0 & D.otherpriors[4] < 0) {
    for (n = 0; n < D.nlai; n++) {
      dn = D.laipts[n];
      tot_exp += pow(log(D.M_LAI[dn] / D.LAI[dn]) / log(2), 2);
    }
    P = P - 0.5 * tot_exp;
  }
  /*use timesteps for mean LAI calculation*/
    ...
    P = P - 0.5 * pow((mlai / (double)D.nlai - D.otherpriors[4]) /
                          D.otherpriorunc[4],
                      2);
    ...
    mlai = mlai / (double)D.nodays;
    /*P=P-0.5*pow((mlai-D.otherpriors[4])/D.otherpriorunc[4],2);}*/
    P = P -
        0.5 * pow(log(mlai / D.otherpriors[4]) / log(D.otherpriorunc[4]), 2);
  }
  /*
  printf("mlai = %2.2f\n",mlai);
  printf("prob =
  ...
  */
  ...
  if (D.net > 0) {
    ...
    if (D.etiav < 1) {
      ...
      for (n = 0; n < D.net; n++) {
        dn = D.etpts[n];
        tot_exp +=
            pow(log(D.M_FLUXES[dn * D.nofluxes + 28] / D.ET[dn]) / log(2), 2);
      }
    } else {
      ...
      for (m = 0; m < D.net / 12; m++) {
        ...
        for (n = 0; n < 12; n++) {
          dn = D.etpts[n + m * 12];
          mam = mam + D.M_FLUXES[dn * D.nofluxes + 28];
          am = am + D.ET[dn];
        }
        ...
        for (n = 0; n < 12; n++) {
          dn = D.etpts[n + m * 12];
          tot_exp +=
              pow(log(D.M_FLUXES[dn * D.nofluxes + 28] / D.ET[dn] * am / mam) /
                      log(2),
                  2);
        }
  ...
  if (D.nnee > 0) {
    ...
    if (D.neeiav < 0) {
      ...
      for (n = 0; n < D.nnee; n++) {
        dn = D.neepts[n];
        tot_exp += pow((D.M_NEE[dn] - D.NEE[dn]) / D.nee_obs_unc, 2);
      }
    } else {
      /*Decoupling seasonal from interannual variations*/
      /*Only use with monthly resolution fluxes, complete years & no missing
      ...
      for (m = 0; m < D.nnee / 12; m++) {
        ...
        for (n = 0; n < 12; n++) {
          dn = D.neepts[n + m * 12];
          tot_exp +=
              pow((D.M_NEE[dn] - D.NEE[dn] - mam + am) / D.nee_obs_unc, 2);
        }
        /*Calculate annual cost function*/
  ...
  /*for
   * (n=1;n<D.nwoo;n++){dn=D.woopts[n];tot_exp+=pow(log((D.M_POOLS[D.nopools*dn+3]/D.M_POOLS[D.nopools*D.woopts[0]+3])/D.WOO[dn])/((D.WOO[dn]-D.WOO[D.woopts[0]])*log(2)),2);}*/
  tot_exp = 0;
  if (D.nwoo > 0) {
    for (n = 0; n < D.nwoo; n++) {
      dn = D.woopts[n];
      tot_exp += pow(log(D.M_POOLS[D.nopools * dn + 3] / D.WOO[dn]) /
                         log(D.otherpriorunc[1]),
                     2);
    }
    P = P - 0.5 * tot_exp;
  }
  ...
  if (D.newt > 0) {
    ...
    for (n = 0; n < D.newt; n++) {
      dn = D.ewtpts[n];
      tot_exp +=
          pow((D.M_POOLS[D.nopools * dn + 6] + D.M_POOLS[D.nopools * dn + 7] -
               D.EWT[dn] - mewtm + mewt) /
                  50,
              2);
    }
  ...
  */
  ...
  if (D.ID == 806) {
    if (D.nband1 > 0) {
      ...
      for (n = 0; n < D.nband1; n++) {
        dn = D.band1pts[n];
        bandmod[n] = D.M_POOLS[D.nopools * dn + 1] * D.M_PARS[33] / mfol +
                     D.M_POOLS[D.nopools * dn + 3] * (1 - D.M_PARS[33]) / mwoo;
      }
    ...
    if (D.nband2 > 0) {
      ...
      for (n = 0; n < D.nband2; n++) {
        dn = D.band2pts[n];
        bandmod[n] = D.M_POOLS[D.nopools * dn + 1] * D.M_PARS[34] / mfol +
                     D.M_POOLS[D.nopools * dn + 3] * (1 - D.M_PARS[34]) / mwoo;
      }
  ...
  if (D.otherpriors[2] > -9999) {
    ...
    for (n = 0; n < D.nodays; n++) {
      mfire += D.M_FLUXES[n * D.nofluxes + 16];
    }
    /*Normalize fire constraint to daily mean flux*/
    mfire = mfire / ((double)D.nodays);

    /*Step 2. Constrain against fire emissions*/
    if (D.otherpriorunc[2] > 0) {
      P = P -
          0.5 * pow(log(mfire / D.otherpriors[2]) / log(D.otherpriorunc[2]), 2);
    } else {
      P = P - 0.5 * pow((mfire - D.otherpriors[2]) / D.otherpriorunc[2], 2);
    }
  }

  /*Constrain mean GPP*/
  if (D.otherpriors[5] > -9999) {
    mgpp = 0;
    /*Step 1. Sum fire emissions*/
    for (n = 0; n < D.nodays; n++) {
      mgpp += D.M_FLUXES[n * D.nofluxes + 0];
    }
    /*Normalize gpp constraint to daily mean flux*/
    mgpp = mgpp / ((double)D.nodays);
    /*Step 2. Constrain against gpp*/
    P = P -
        0.5 * pow(log(mgpp / D.otherpriors[5]) / log(D.otherpriorunc[5]), 2);
  ...
  if (D.otherpriors[6] > -9999) {
    mnpdf = 0;
    /*Step 1. Sum of biomass -> litter emissions (including fire)*/
    for (n = 0; n < D.nodays; n++) {
      for (f = 0; f < 7; f++) {
        mnpdf += D.M_FLUXES[n * D.nofluxes + npdfi[f]];
      }
    }
    ...
    if (mnpdf < D.otherpriors[6]) {
      P = P -
          0.5 * pow(log(mnpdf / D.otherpriors[6]) / log(D.otherpriorunc[6]), 2);
    }
  }
