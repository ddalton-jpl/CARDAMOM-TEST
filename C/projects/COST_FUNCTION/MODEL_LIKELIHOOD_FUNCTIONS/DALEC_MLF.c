...
double DALEC_MLF(DATA DATA, double *PARS) {
  ...
  */
  ...
  if (EDC == 1 | EDCD.DIAG == 1) {
    ...
    /*storing GPP*/
    // for
    // (n=0;n<DATA.nodays;n++){DATA.M_GPP[n]=DATA.M_FLUXES[n*DATA.nofluxes];}
  ...
  if (DATA.ncdf_data.EDCDIAG == 1) {
    for (n = 0; n < 100; n++) {
      DATA.M_EDCD[n] = EDCD.PASSFAIL[n];
    }
  }
...
int DALEC_MLF_MODCONFIG(MLF *MLF) {
  ...
  DALEC_ALL_LIKELIHOOD_MODCONFIG(&LIKELIHOODinfo);
  // Store LS
...
double EDC_DALEC_MLF_STEPWISE(DATA DATA, double *PARS) {
  ...
  /*Independend EDCD structure: stores detailed EDC info, and provides DATA with
   * result summary*/
  struct EDCDIAGNOSTIC EDCD;
