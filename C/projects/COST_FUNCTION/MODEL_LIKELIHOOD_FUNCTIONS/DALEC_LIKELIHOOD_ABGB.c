double DALEC_LIKELIHOOD_ABGB(DATA D) {
  ...
  if (D.otherpriors[0] > -9999) {
    ...
    P = P - 0.5 * pow(log((D.M_ABGB_t0[0]) / D.otherpriors[0]) /
                          log(D.otherpriorunc[0]),
                      2);
  }
  ...
  if (D.nabgb > 0) {
    ...
    // tot_exp+=pow(log(max(biomass,0.01)/max(D.WOO[dn],0.01))/log(max(D.otherpriorunc[1],0.1)),2);}
    // /*shuang for correcting -inf*/
