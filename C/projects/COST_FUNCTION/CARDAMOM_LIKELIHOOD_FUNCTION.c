...
typedef struct TIMESERIES_OBS_STRUCT {
  ...
  int opt_unc_type; //(0 = absolute sigma, 1 = uncertainty factor, 2 = sigma as
                    // fraction of value)
  int opt_normalization; //(0 = none, 1 = remove mean, 2 = divide by mean)
  int opt_filter; //(0 = no filter, 1 = mean only, 2==annual mean & monthly
                  // anomaly, 3 = annual only).
  double min_threshold; // Minimum value threshold: model and/or data will be
                        // rounded up to this value (default = -inf)
  double single_monthly_unc; // Fields to be used only with Filter=2 AND
                             // opt_unc_type=0;
  double single_annual_unc;  // Fields to be used only with Filters=2 & 3 (AND
                            // opt_unc_type=0 or opt_unc_type=2 for filter = 2);
...
double TIMESERIES_OBS_STRUCT_PREPROCESS(TIMESERIES_OBS_STRUCT *OBS) {
  ...
  // Global defaults for timeseries obs... these are only defined if they
  // haven't been defined earlier
  ...
  for (k = 0; k < OBS->valid_obs_length; k++) {
    ...
    if (OBS->unc[OBS->valid_obs_indices[k]] == DEFAULT_DOUBLE_VAL) {
      if (OBS->opt_unc_type < 2) {
        ...
        OBS->unc[OBS->valid_obs_indices[k]] =
            max(OBS->single_unc * OBS->values[OBS->valid_obs_indices[k]],
                OBS->single_unc * OBS->min_threshold);
      }
    }
  }
  ...
  for (k = 0; k < OBS->valid_obs_length; k++) {
    OBS->unc[OBS->valid_obs_indices[k]] =
        sqrt(pow(OBS->unc[OBS->valid_obs_indices[k]], 2) +
             pow(OBS->structural_unc, 2));
  }
...
TIMESERIES_OBS_STRUCT READ_NETCDF_TIMESERIES_OBS_FIELDS(int ncid,
                                                        char *OBSNAME) {
  ...
  if (OBS.unc_length == 0) {
    ...
    int n;
    for (n = 0; n < OBS.unc_length; n++) {
      OBS.unc[n] = DEFAULT_DOUBLE_VAL;
    }
  }

  OBS.opt_unc_type = ncdf_read_int_attr(
      ncid, OBSNAME, "opt_unc_type"); // absolute, log, percentage
  OBS.opt_normalization =
      ncdf_read_int_attr(ncid, OBSNAME, "opt_normalization");
  ...
  OBS.single_monthly_unc =
      ncdf_read_double_attr(ncid, OBSNAME, "single_monthly_unc");
  OBS.single_annual_unc =
      ncdf_read_double_attr(ncid, OBSNAME, "single_annual_unc");
...
// typedef struct SINGLE_OBS_STRUCT{
// double value;
// double unc;//(0 = absolute sigma, 1 = uncertainty factor, 2 = sigma as
// fraction of value) int opt_unc_type;//log-transform data double
// min_threshold; }SINGLE_OBS_STRUCT;
//
...
SINGLE_OBS_STRUCT READ_NETCDF_SINGLE_OBS_FIELDS(int ncid, char *OBSNAME) {
  ...
  // //MINMAX LIMITS: OBS.min_value=ncdf_read_double_attr(ncid,
  // OBSNAME,"min_value");
  ////MINMAX LIMITS: OBS.max_value=ncdf_read_double_attr(ncid,
  /// OBSNAME,"max_value");
  ...
  // isempty flag
  OBS.validobs = false;
  if (OBS.min_value != DEFAULT_DOUBLE_VAL) {
    OBS.validobs = true;
  }
  if (OBS.max_value != DEFAULT_DOUBLE_VAL) {
    OBS.validobs = true;
  }
...
double CARDAMOM_TIMESERIES_OBS_LIKELIHOOD(TIMESERIES_OBS_STRUCT *OBS,
                                          double *MOD) {
  ...
  if (OBS->valid_obs_length > 0) {
    ...
    */
    ...
    // threshold-- applies after normalization, before log-transform (if
    // selected)
    ...
    // Cost function
    // This is the only option available for single value (e.g. time invariant)
    // observations
    if (OBS->opt_filter == 0) { // no filter
    ...
    else if (OBS->opt_filter == 2 |
             OBS->opt_filter == 3) { // monthly and annual flux
      /*Decoupling seasonal from interannual variations*/
      /*Only use with monthly resolution fluxes, complete years & no missing
       * data*/
      ...
      for (m = 0; m < N / 12; m++) {
        ...
        if (OBS->opt_filter == 2) {
          for (n = 0; n < 12; n++) {
            dn = n + m * 12;
            tot_exp +=
                pow((mod[dn] - obs[dn] - mam + oam) / single_monthly_unc, 2);
          }
        }
        /*Calculate annual cost function*/
        /*TEST: normalize model likelihood by normal distribution with mean zero
         * and unc = x2 annual unc.*/
        tot_exp += pow((oam - mam) / single_annual_unc, 2);
      }
    }

    else if (OBS->opt_filter == 4 |
             OBS->opt_filter ==
                 5) { // climatology and inter-annual variability, only to use
                      // with monthly data
      // Decoupling climatological seasonal cycle from inter-anual variations*/
      // Only use with monthly resolution fluxes, complete years & no missing
      // data*/ Step 1. Mean model & data annual NBE*/ Step 2. Compare means*/
      // Step 3. Remove means from months for cost function*/
      int m, dn, i;
      ...
      for (m = 0; m < N; m++) {
        // Obs index from 1-12
        icm[m] = OBS->valid_obs_indices[m] % 12;
        i = icm[m];
      ...
      // Climatological cost function component: Avoid averaging by scaling cost
      // function uncertainties accordingly!
      for (i = 0; i < 12; i++) {
        if (countcm[i] > 0) {
      ...
      // Inter-annual cost function component: Avoid averaging by scaling cost
      // function uncertainties accordingly!
      if (OBS->opt_filter == 4) {
        for (m = 0; m < N; m++) {
          i = icm[m];
          if (countcm[i] > 1) {
            tot_exp += pow(
                (mod[m] - modcm[i] - obs[m] + obscm[i]) / single_annual_unc, 2);
          }
        }
      }
      // Free pointers
      ...
      // EB added 12.12.22 as a test to constrain biomass trend with reduced
      // noise: constrain fit to 3 year rolling mean obs/
      int m, dn;
...
double CARDAMOM_SINGLE_OBS_LIKELIHOOD(SINGLE_OBS_STRUCT *OBS, double MOD) {
  ...
  if (OBS->value != DEFAULT_DOUBLE_VAL) {
    ...
    // MINMAX LIMITS if (mod < OBS->min_value || mod > OBS->max_value
    // ){P=log(0);}
  }
