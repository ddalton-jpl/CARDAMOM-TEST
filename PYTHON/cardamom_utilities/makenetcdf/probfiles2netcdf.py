...
def makeblank_geoschem(pts, dictset=None):
    ...
    da = xr.DataArray(
        data=nullmat.copy(), coords=[latcoord, loncoord, ensemcord], dims=dims
    )
...
def filename_to_wildcard(filelist, uniquename=None):
    wildcard = list(
        set(
            [
                "_".join(s.split("/")[-1].split(".")[0].split("_")[1:-2])
                for s in filelist
            ]
        )
    )
...
def probfiles_to_netcdf(filelist, cutfrac=0.5):
    """
    ...
    for cw in pts:
        ...
        ds[varnotime].loc[
            dict(lat=lat, lon=lon, ensemble=slice(1, np.array(probs).shape[0]))
...
if __name__ == "__main__":
    ...
    dstest = probfiles_to_netcdf(filelist, cutfrac=0.5)

    dstest.to_netcdf(
        diroutput + "SUMMARYSTAT1_probfile_cru004GCR006_1920_2015_nbe2002.nc"
    )
