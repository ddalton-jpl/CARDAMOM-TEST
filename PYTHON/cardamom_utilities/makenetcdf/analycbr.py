...
def cbr_to_netcdf(filelist, cutfrac=0.5, INFO=[], filesize=132000, modelid=811):
    """
    ...
    da = xr.DataArray(
        data=nullmat.copy(),
        coords=[latcoord, loncoord, ensemcord],
    ...
    for cw in ptsincommon:
        ...
        for pn in rcardmdl.get_parnames(modelid, output="shortnames"):
            ds[pn].loc[dict(lat=lat, lon=lon)] = np.array(
                pars[:, rcardmdl.get_parnames(modelid, output="shortnames").index(pn)]
            )
            ds[pn].attrs = {
                "longname": rcardmdl.get_parnames(modelid=modelid, output="longnames")[
