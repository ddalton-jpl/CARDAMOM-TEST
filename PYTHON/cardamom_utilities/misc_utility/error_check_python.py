            ...
            if hasattr(current_var, "single_unc"):
                ...
                if opt_unc_type_value == 1 and single_unc_value < 1:
                    print(
                        "Error: Variable "
                        + current_name
                        + " has invalid combination of opt_unc_type==1 and single_unc<1"
                    )
            ...
            if opt_unc_type_value == 1:
                ...
                if numpy.any(data) <= 0:
                    print(
                        "Error: Variable "
                        + current_name
                        + " has invalid combination of opt_unc_type==1 and values<=0"
                    )
                    obs_attribute_errors = obs_attribute_errors + 1
        # check opt_filter
            ...
            if hasattr(current_var, "single_mean_unc"):
                ...
                if opt_filter_value == 1 and single_mean_unc == -9999:
                    print(
                        "Error: Variable "
                        + current_name
                        + " has invalid combination of opt_filter="
                        + str(opt_filter_value)
                        + " and single_mean_unc="
                        + str(single_mean_unc)
                    )
                    obs_attribute_errors = obs_attribute_errors + 1
            ...
            if hasattr(current_var, "single_annual_unc"):
                ...
                if opt_filter_value == 2 and single_annual_unc == -9999:
                    print(
                        ...
                        + " has invalid combination of opt_filter="
                        + str(opt_filter_value)
                        + " and single_annual_unc="
                        + str(single_annual_unc)
                    )
                    obs_attribute_errors = obs_attribute_errors + 1
            ...
            if hasattr(current_var, "single_monthly_unc"):
                ...
                if opt_filter_value == 2 and single_monthly_unc == -9999:
                    print(
                        ...
                        + " has invalid combination of opt_filter="
                        + str(opt_filter_value)
                        + " and single_monthly_unc="
                        + str(single_monthly_unc)
                    )
                    obs_attribute_errors = obs_attribute_errors + 1
...
def check_variable(CBF, var):
    if var in CBF.variables:
        ...
        if result_temp is True:
            warnings.warn(
                "CBF OBS {} is negative, file will make CARDAMOM crash".format(var)
            )
            return 1
        else:
            print("OK\n")
...
def check_mean_variable(CBF, var):
    if var in CBF.variables:
        print(
            "Checking CBF OTHER OBS {} is positive or valid missing value".format(var)
        )
        temp = CBF[var][:]
        if temp <= 0:
            if temp != -9999:
                warnings.warn(
                    "CBF OTHER OBS {} is negative, file will make CARDAMOM crash".format(
                        var
                    )
                )
                return 1
...
def check_driver_variable(CBF, var):
    if var in CBF.variables:
        ...
        if flag_non_na_inf == 0:
            warnings.warn(
                "CBF Driver {} contains Nan or Inf values, file will make CARDAMOM crash".format(
                    var
                )
            )
            return 1
            ...
            if flag_non_negative == 0:
                warnings.warn(
                    "Driver {} not within physical range, file will make CARDAMOM crash".format(
                        var
                    )
                )
                return 1
...
def check_temperature_vars(CBF, var):
    if var in CBF.variables:
        ...
        if flag_non_na_inf == 0:
            ...
            temp[temp == -9999] = 9999
            ar_negative = any(
                (numpy.array(temp) + 273.15) < 0
            )  # convert to kelvin and check anything < 0 kelvin
            if ar_negative == False:
                print("OK\n")
    ...
    for name, variable in CBF.variables.items():
        ...
        if name not in toexclude:
            ...
            if (
                temp_data.size == 1
                and temp_data != -9999
                and valid_value_check(temp_data) == 1
            ):
                if hasattr(current_var, "unc"):
                    ...
                    if unc_value == -9999:
                        print(
                            "Error: Variable "
                            + current_name
                            + " has invalid combination of length==1 and unc equal to -9999"
                        )
                        vars_unc_errors = vars_unc_errors + 1
                    elif (
                        numpy.isnan(unc_value) == True or numpy.isinf(unc_value) == True
                    ):
                        print(
                            "Error: Variable "
                            + current_name
                            + " has invalid combination of length==1 and unc equal to Inf or NaN"
                        )
                        vars_unc_errors = vars_unc_errors + 1
                else:
                    print(
                        "Error: Variable "
                        + current_name
                        + " has invalid combination of length==1 and missing attribute unc"
                    )
                    vars_unc_errors = vars_unc_errors + 1
        ...
        if hasattr(current_var, "_FillValue"):
            ...
            if current_FillVal != -9999:
                ...
                print(
                    "Error: Variable "
                    + current_name
                    + " has invalid fill value and not equal to -9999"
                )
...
def main():
    parser = argparse.ArgumentParser(
        ...
        description="This tool checks netcdf cbf files",
    )
    parser.add_argument(
        "--cbf_file",
        "-c",
        help="Path to the netcdf cbf output file to test",
        default=None,
    )
    ...
    if obs_attribute_flag > 0:
        print(
            "OBS attribute check failed with total "
            + str(obs_attribute_flag)
            + " error(s)"
        )
        error_message_summary = (
            error_message_summary
    ...
    if driver_flag > 0:
        ...
        error_message_summary = (
            error_message_summary
            + "\n Driver check failed with total "
            + str(driver_flag)
            + " warning(s)"
        )
    ...
    if unc_flag > 0:
        ...
        error_message_summary = (
            error_message_summary
            + "\n unc attribute check failed with total "
            + str(unc_flag)
            + " error(s)"
        )
    ...
    if fill_value_flag > 0:
        ...
        error_message_summary = (
            error_message_summary
            + "\n fill value check failed with total "
            + str(fill_value_flag)
            + " error(s)"
        )
    ...
    if len(error_message_summary) > 0:
        print(
            "\nError Summary: Total "
            + str(total_errors)
            + " Errors/Warnings Found. See below:"
            + error_message_summary
        )
    ...
    print(
        "Tip: check that all met fields required for model ID are provided in cbf.nc file"
    )
