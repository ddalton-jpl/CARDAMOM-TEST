...
def library_parameter(modelid, modeldir):
    parameterfile = (
        "projects/CARDAMOM_MODELS/DALEC/"
        + "DALEC_"
        + str(modelid)
        + "/"
        + "PARS_INFO_"
                ...
                bb = A[
                    ...
                    - (
                        ...
                        - np.max(
                            [
                                idxx
                                for idxx, aa in enumerate(A[idx - 4 : idx + 1])
                                if aa.strip() == ""
                            ]
                        )
                    )
...
def get_parnames(
    modelid, output="shortnames", modeldirshort="/repos/models/cardamom/C/"
):
