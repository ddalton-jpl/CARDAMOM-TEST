...
def define_cbf_obs_fields(CBF, CBFOBS):
    ...
    CBF["OBSinfo"] = {}
    CBF["OBSinfo"]["LAI"] = (
        "LAI data requirements: should be +ve definite (>0);"
        + "assumed uncertainty for LAI timeseries is factor of 2;"
        + "future versions will include user-defined uncertainty options"
    )
    CBF["OBSinfo"]["uncertainty_factors"] = (
        "Uncertainty structures for positive-definite"
...
def read_obs_uncertainty_fields(CBF, SD, OPRU):
    ...
    CBF["OBSUNC"]["NBE"][
        "info"
    ] = "Single point (default = 0.5, must be >0) and annual (annual_unc) NBE uncertainty [gC/m2/d]"
    ...
    CBF["OBSUNC"]["EWT"][
        "info"
    ] = "Single point (default = 50) and annual (N/A yet) EWT uncertainty [mm]"
    ...
    CBF["OBSUNC"]["GPP"][
        "gppabs_info"
    ] = 'Set to "1" for GPP data, set to "0" for SIF data"'
    ...
    CBF["OBSUNC"]["SOM"][
        "info"
    ] = "Single point uncertainty on time-resolved SOM, CBF.OBS.SOM"
...
def read_other_obs_constraints(CBF, OPR, OPRU):
    ...
    CBF["OTHER_OBS"]["MBiomass"]["info"] = (
        "Mean total (above-& below- ground) biomass at time t=0\n"
    ...
    CBF["OTHER_OBS"]["MFire"]["info"] = (
        "Mean total (above-& below- ground) fire emissions "
    ...
    CBF["OTHER_OBS"]["MLAI"]["info"] = (
        "Mean total LAI for whole simulation \n "
    ...
    CBF["OTHER_OBS"]["MGPP"]["info"] = (
        "Mean total (above-& below- ground) GPP "
        + "for whole simulation \n Use +ve value for "
...
def read_cbf_file(inputfilename=None):
    ...
    TEMPDATA = BD[k : k + (CBF["nomet"] + CBF["noobs"]) * CBF["nodays"]].reshape(
        CBF["nodays"], (CBF["nomet"] + CBF["noobs"])
    )
    # All met data
    CBF["MET"] = TEMPDATA[
        0 : CBF["nodays"], 0 : CBF["nomet"]
    ]  # Add in new meteorology here
    ...
    if (
        len(BD) - (k + (CBF["nomet"] + CBF["noobs"]) * CBF["nodays"])
        == CBF["nomet"] + CBF["noobs"]
    ):
    ...
    CBF["RAW"][
        "details"
    ] = "For completeness & development purpose only; When re-writing CBF to file, these are over-written by CBF.OBS, etc."
...
def compile_cbf_obs_fields(CBF):
    ...
    if "OBS" in CBF.keys():
        if type(CBF["OBS"]) == np.ndarray:
            CBFOBS = CBF["OBS"]
            warnings.warn(
                "CARDAMOM_WRITE_BINARY_FILEFORMAT: CBF.OBS matrix used to define observations (will soon be obsolete)",
                DeprecationWarning,
            )
            ...
            for n in np.arange(len(isobs))[isobs]:
                ...
                if (
                    not not list(CBF["OBS"][obsnames[n]])
                    and np.shape(CBF["OBS"][obsnames[n]])[0] == N
                ):
                    CBFOBS[:, n] = CBF["OBS"][obsnames[n]]
                if np.shape(CBF["OBS"][obsnames[n]])[0] != N and not not list(
                    CBF["OBS"][obsnames[n]]
                ):
                    warnings.warn(
                        "CBF[OBS] field dimensions not compatible with CBF.MET, writing -9999"
                    )
...
def write_obs_uncertainty_fields(CBF, SD, OPRU):
    ...
    if (
        type(CBF["OBS"]) == dict
        and "ABGB" in CBF["OBS"].keys()
        and not not list(CBF["OBS"]["ABGB"])
        and CBF["OBSUNC"]["ABGB"]["unc"] == -9999
    ):
        print("Need to prescribe CBF.AGBGunc field")
    if (
        type(CBF["OBS"]) == dict
        and "SOM" in CBF["OBS"].keys()
        and not not list(CBF["OBS"]["SOM"])
        and CBF["OBSUNC"]["SOM"]["unc"] == -9999
    ):
...
def CARDAMOM_WRITE_BINARY_FILEFORMAT(CBF, outfilename):
    ...
    if CBF["nodays"] != nodays:
        warnings.warn(
            "CBF.nodays updated for consistency with CBF.MET and CBF.OBS dimensions"
        )
        CBF["nodays"] = nodays

    # Must update if new information is available
    SD[0:7] = np.expand_dims(
        np.array(
            [
                CBF["ID"],
                CBF["LAT"],
    ...
    if "mmet" in CBF.keys() and CBF["mmet"].shape[0] == CBF["nomet"]:
        MTEMPDATA = np.expand_dims(
            np.concatenate((CBF["mmet"], np.ones((CBF["noobs"])) * fill_value)), axis=0
        )
    ...
    if (
        TEMPDATA.shape[0]
        != (CBF["nomet"] + CBF["noobs"]) * CBF["nodays"] + MTEMPDATA.shape[1]
    ):
...
def CARDAMOM_READ_OUTPUT(
    cbffile, cbrfile, fluxfile, poolfile, probfile=[], baddata=-9999, INFO=[]
):
    ...
    model_info = {"nopars": nopars, "latterhalf": 0}
...
def CARDAMOM_READ_OUTPUT_FILEAWARE(
    cbffile, cbrfiles, outputfiles, cutfrac=0.5, INFO=[], chaininfoout=None
):
    ...
    if "probfile" in namedvars:
        ...
        probname = "_".join(
            probfilelist[0].split("/")[-1].split(".")[0].split("_")[:-1]
        )
    ...
    while count < len(runlist) - 1:
        if count == 0:
            runnums = [
                s.split("/")[-1].split(".")[0].split("_")[-1] for s in runlist[count]
            ]
        ...
        runnums = list(
            ...
            & set(
                [
                    s.split("/")[-1].split(".")[0].split("_")[-1]
                    for s in runlist[count + 1]
                ]
            )
        )
        count = count + 1
    ...
    for idx, rn in enumerate(runnums):
        ...
        if "probfile" in namedvars:
            ...
            CBRtmp = CARDAMOM_READ_OUTPUT(
                cbffile, cbrfile, fluxfile, poolfile, probfile, INFO
            )
...
if __name__ == "__main__":
    ...
    cbrname = (
        "CBFerai_2001-2016_obsmod_icabg_fmodis_clstr_11_divide_median_rndm_2042_0.cbr"
    )
    cbrfile = testcbrdir + cbrname
